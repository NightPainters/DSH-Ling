...
export const SCAN_DEFAULTS = Object.freeze({
  minHits: 3,        // 同类纠正至少出现几次
  minSessions: 2,    // 至少跨几个会话(单次会话里被骂三遍不算"模式")
...
export function scanCorrections(memory, opts = {}) {
  ...
  try {
    rows = memory.db.prepare(
      "SELECT session_id, text FROM dsh_turns_raw WHERE role='user' ORDER BY seq DESC LIMIT ?",
    ).all(maxTurns);
  } catch {
...
export function buildReflectMaterial(memory, opts = {}) {
  ...
  try {
    const total = Number(memory?.db?.prepare?.("SELECT COUNT(*) n FROM dsh_turns_raw WHERE role='user'").get()?.n) || 0;
    stats.turnTotal = total;
    let off = Math.max(0, Number(cursor?.turnOffset) || 0);
    if (total > 0 && off >= total) { off = 0; stats.wrapped = true; }
    const rows = total > 0
      ? (memory.db.prepare("SELECT ts, text FROM dsh_turns_raw WHERE role='user' ORDER BY rowid DESC LIMIT ? OFFSET ?").all(turnLimit, off) || [])
      : [];
    const lines = rows
