// dsh-ling host — 规矩 / 习惯 域逻辑(2026-09-15 与用户定稿的二分)
//
// 原则:**指令直达,人格不直达。**
...
// 待确认队列上限(2026-09-16 用户定案:5)。
// 理由:珍贵的东西不该被摆成流水线 —— 满了就**明确驳回**新的生成请求并说明,而不是排队等她一辈子。
export const PENDING_MAX_DEFAULT = 5;
 ...
 *  1.5.2 起对外导出:渲染出口(persona.js)复用同一把尺,禁止另发明第二套归一化。 */
...
/** 待确认队列上限(可由 settings.habits.pendingMax 覆盖)。 */
export function pendingMaxOf(settings) {
  const root = settings?.get ? settings.get() : (settings || {});
  const n = Number(root?.habits?.pendingMax);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : PENDING_MAX_DEFAULT;
}
...
export function rulesView(settings) {
  ...
  return {
    ...
    caps: { rules: RULES_MAX, habits: HABITS_MAX, pending: pendingMaxOf(settings) },
  };
}
...
export async function proposeHabit({ settings, habit, evidence = '', byUser = false, now = Date.now() }) {
  ...
  // 上限:满了就驳回,不再往里塞(2026-09-16 定案)—— 让她"等着"才是最伤人的那种安排。
  const cap = pendingMaxOf(settings);
  if (pending.length >= cap) return { ok: false, reason: 'pending-full', limit: cap, pending: pending.length };
