...
export class MemoryStore {
  ...
  logBranch(branchId, action, { before = '', after = '', note = '' } = {}) {
    try {
      this.db.prepare(`INSERT INTO branch_log (at,branch_id,action,before_val,after_val,note)
        VALUES (?,?,?,?,?,?)`)
        .run(utcIso(), String(branchId || ''), String(action || '').slice(0, 40),
          String(before ?? '').slice(0, 200), String(after ?? '').slice(0, 200),
  ...
  branchLogDigest({ limit = 5 } = {}) {
    ...
    const lines = fresh.slice(0, max).map((r) => {
      ...
      switch (String(r.action || '')) {
        ...
        case 'gather': return '把「' + who + '」归并到了「' + r.after + '」下';
        case 'autobuild': return '自动建了一条枝「' + r.after + '」' + (r.note ? '(' + r.note + ')' : '');
  ...
  restoreTree(snapshotId) {
    ...
    try {
      ...
      const now = utcIso();
      this.db.exec('BEGIN');
      this.db.prepare('DELETE FROM branch WHERE id<>?').run(TRUNK_ID);
    ...
    this.logBranch(TRUNK_ID, 'restore', { after: String(row.name || ('#' + sid)), note: '恢复树快照 #' + sid });
    return { ok: true, id: sid, backupId: backup.ok ? backup.id : null, counts: backup.ok ? backup.counts : null };
  }
  ...
  branchCounts() {
    ...
    let total = 0;
    for (const r of rows) {
      byBranch[String(r.bid)] = Number(r.n);
      total += Number(r.n);
    }
