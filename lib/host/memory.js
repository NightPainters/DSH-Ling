 ...
 *  在复盘留痕里一律显示成「主人刚改动了记忆树」,主人复盘时看到的是自己没做过的操作
...
export class MemoryStore {
  ...
  /** 打遗忘标记(归档由调用方先行完成;此处只写库)。 */
  forgetEntry({ source, convId, reason = '', archivePath = '', actor = 'user' } = {}) {
  ...
  resolveVeinSuggestion(id, { action = 'accept', text = '', note = '' } = {}) {
    ...
    try {
      ...
      return { ok: true, id: sid, status: 'accepted', veinId, chars: body.length, branchId: veinId };
    } catch (e) {
      return { ok: false, reason: 'db', error: String(e?.message ?? e).slice(0, 160) };
    }
  }
