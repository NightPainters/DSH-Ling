...
import { ensureDir, sha256Text, utcIso, mergeDeep } from './util.js';

const SCHEMA_VERSION = 2; // v2:conv_overview.source 放开 CHECK(支持 dsh/import 等第三方来源)
...
export class MemoryStore {
    ...
    this.db.prepare(
      `INSERT INTO conv_overview
       ...
       ON CONFLICT(source, conv_id) DO UPDATE SET
         ...
         summary=excluded.summary, heat=excluded.heat, importance=excluded.importance,
  ...
  listOverviews({ limit, onlyOk, source } = {}) {
    ...
    sql += ' ORDER BY heat DESC, updated_at DESC';
  ...
  queryOverviews({ source, category, q, sort = 'updated', limit = 100, offset = 0 } = {}) {
    ...
    const orderSql = sort === 'created' ? 'started_at DESC' : sort === 'heat' ? 'heat DESC, importance DESC' : 'updated_at DESC';
  ...
  appendRawTurn(sessionId, { seq, role, ts, model, text }) {
    ...
    ).run(String(sessionId), Number(seq), String(role ?? '?'), ts || null, model || null, text);
    this.db.prepare(
  ...
  lastUserTurnAt() {
    const r = this.db.prepare(
      "SELECT ts FROM dsh_turns_raw WHERE role='user' AND ts IS NOT NULL AND ts<>'' ORDER BY ts DESC LIMIT 1",
    ).get();
    return r ? String(r.ts) : null;
  }
