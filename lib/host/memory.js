...
// A-01(1.5.1):主脉记忆被覆盖前**必须先归档**。archive.js 只依赖 node:fs / node:path,
// 且把 memory 当参数传(依赖注入)⇒ memory.js 引它**不成环**。
import { archiveBefore } from './archive.js';

const SCHEMA_VERSION = 13; // v13:forgotten(软遗忘标记 —— 永远不彻底删除,先归档再打标记);v12:branch_log.actor(G4 器灵侧通道 —— 工具改的树必须记在"器灵"名下,不再一律记成主人);v11:vein_suggestions(主脉提炼审核队列 R7 —— 设计稿 §5:提炼必须走审核,不直接进记忆);
// v10:tree_snapshot.kind('manual'|'auto',审计 A#9 —— 自动备份与手动快照此前外表完全一致,删错了就没有撤回路径);v9:tree_snapshot(树的保存/恢复 R1);v8:conv_branch(非会话源的枝归属覆盖层,方案 A);v7:branch_log(复盘留痕);v6:vein_link + memory_conflict(D9-b 树的生长);v5:branch(记忆分枝 D9-a);v4:title_by(定名者:user/ai);v3:title_locked;v2:conv_overview.source 放开 CHECK
...
CREATE TABLE IF NOT EXISTS branch (
  ...
  -- ⚠️ 预留字段(审计 A#11,2026-09-22):weight_scale / visibility 目前**只存与显示**,
  -- **不参与任何打分**(L1 的 score 只看血缘档位 × 矛盾系数)。让它们真正生效会改变检索排序语义,
  -- 须主人拍板后再动;在那之前返回体里带 applied=false 标记,免得界面把"存了"当成"生效了"。
...
CREATE TABLE IF NOT EXISTS branch_log (
  ...
  note       TEXT NOT NULL DEFAULT '',
  actor      TEXT NOT NULL DEFAULT 'user'   -- v12:'user' | 'ling'(G4:谁改的树)
);
...
CREATE TABLE IF NOT EXISTS tree_snapshot (
  ...
  note       TEXT NOT NULL DEFAULT '',
  data       TEXT NOT NULL,
  kind       TEXT NOT NULL DEFAULT 'manual'   -- v10:'manual' | 'auto'(审计 A#9)
);
CREATE TABLE IF NOT EXISTS vein_suggestions (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  at          TEXT NOT NULL,
...
/** v9→v10:tree_snapshot 增列 kind(审计 A#9,2026-09-22)。
 *
 *  为什么需要:恢复前的那份**自动备份**是这个系统里"撤回上一次恢复"的唯一路径,
 *  但它在库里与主人手动存的快照**外表完全一致**(同名同形同大小)—— 界面上分不出哪个能删,
 *  删掉最后一份 auto 之后,一次误恢复就再也没有回路。
 *
 *  存量行没有 kind 可考,按既有命名习惯**回填**:`restoreTree` 的自动备份一直叫「自动快照 - …」
 *  (迁移只在这一列刚加上时跑一次,幂等)。 */
function migrateSnapshotKind(db) {
  try {
    const cols = db.prepare('PRAGMA table_info(tree_snapshot)').all().map((c) => String(c.name));
    if (cols.includes('kind')) return; // 已迁移
    db.exec("ALTER TABLE tree_snapshot ADD COLUMN kind TEXT NOT NULL DEFAULT 'manual'");
    db.prepare("UPDATE tree_snapshot SET kind='auto' WHERE name LIKE '自动快照%'").run();
  } catch (e) {
    console.warn('[dsh-ling] tree_snapshot.kind 迁移失败(忽略):', String(e?.message ?? e).slice(0, 200));
  }
}

/** v11→v12:branch_log 增列 actor(G4 器灵侧正规通道,2026-09-25)。
 *
 *  为什么需要:Part G 之前,器灵想改记忆树**没有任何正规通道** —— 只能伪造本机进程身份走
 *  UI 的 HTTP 口,或者直接开库。而 `logBranch` 又**没有 actor 参数**,于是那些改动
 *  在复盘留痕里**一律记在主人名下**,主人复盘时看到的是自己没做过的操作
 *  (实测 91 笔)。加上 actor 后,工具通道的改动记 `'ling'`,注入文案据此分流。
 *
 *  存量行没有 actor 可考 —— 默认回填 `'user'`(旧通道确实都是人经界面点的)。 */
function migrateLogActor(db) {
  try {
    const cols = db.prepare('PRAGMA table_info(branch_log)').all().map((c) => String(c.name));
    if (cols.includes('actor')) return; // 已迁移
    db.exec("ALTER TABLE branch_log ADD COLUMN actor TEXT NOT NULL DEFAULT 'user'");
  } catch (e) {
    console.warn('[dsh-ling] branch_log.actor 迁移失败(忽略):', String(e?.message ?? e).slice(0, 200));
  }
}

/** 会话级成员"可展开"的谓词(审计 A#6,2026-09-22)。
 *
 *  展开(`branchMembers`)与计数(`branchMemberCounts` / `branchTree().members`)必须共用这一份 SQL,
 *  否则计数会变成"永远展不开的假广告":旧实现 `branchTree` 直接数 `session_meta` 全量,
 *  而 `branchMembers` 会滤掉**空壳会话**(既无概述标题、又没有任何原文轮次)——
 *  实测主干写「76 个会话」,点开只有 15 条,剩下 61 条永远点不开。
 *  两处共用字符串常量,比"两处各手写一遍同一个 WHERE"更难写歪。 */
const MEMBER_LIVE_SESSION = "(NULLIF(TRIM(o.title),'') IS NOT NULL OR EXISTS (SELECT 1 FROM dsh_turns_raw t2 WHERE t2.session_id = m.session_id))";
const MEMBER_LIVE_ENTRY = "(NULLIF(TRIM(o.title),'') IS NOT NULL OR EXISTS (SELECT 1 FROM dsh_turns_raw t2 WHERE t2.session_id = c.conv_id))";

/** 计数 Map → **双读法**视图(审计 A#6 跨文件对接,2026-09-22)。
 *
 *  为什么不能只给 Map:api.js 的 `memberCountOf()` 是"软链"消费方,它按
 *  `Object.keys(计数).forEach(id => 计数[id])` 的**下标**读法取数 —— 而 `Object.keys(Map)` 恒为 `[]`
 *  (Map 的键不是自有可枚举属性),于是它不会报错,只会**静默地把每条枝的计数读成 0**,
 *  端点上还照样标着"口径 = members"。这种"形状不匹配 ⇒ 全 0 且不报错"是最难查的一类。
 *  故返回普通对象(下标读法可用),并把 `.get()` 以**不可枚举**属性附上(Map 读法也可用,
 *  且不会出现在 `Object.keys` 里污染消费者)。
 */
function countView(m) {
  const o = {};
  for (const [k, v] of m) o[k] = v;
  Object.defineProperty(o, 'get', { value: (k) => o[String(k)], enumerable: false });
  return o;
}
...
export class MemoryStore {
  constructor(dbPath) {
    ...
    this.dbPath = dbPath; // archive.js 用它推导归档目录(与库同级)
    ...
    migrateSnapshotKind(this.db); // v9→v10:tree_snapshot.kind(区分自动备份与手动快照,审计 A#9)
    migrateLogActor(this.db); // v11→v12:branch_log.actor(区分"谁改的树",Part G G4)
    ...
    // v10 加 tree_snapshot.kind(ALTER,见 migrateSnapshotKind)。
   ...
   *  ⚠️ `visibility` 是**预留字段**(审计 A#11,2026-09-22):只落库、只显示,
   *  当前不参与血缘解析、不参与检索打分(血缘看的是树结构,不是这个字段)。 */
  ...
  /** 全部枝(含主干)。
   *
   *  ⚠️ 预留字段(审计 A#11,2026-09-22 定为方案 b):`weightScale` / `visibility`
   *  目前**只存只显示,不参与任何打分** —— L1 的分数是 `raw × 血缘档位 × 矛盾系数`,
   *  血缘档位由 `lineageWeightMap` 按树结构算(1.0 / 0.7 / 0.4),与这两个字段无关。
   *  改语义(让权重真正乘进检索分)会改变所有既有记忆的排序结果,**需主人拍板**,
   *  故这里不擅自改打分,只在返回体里如实标明 `weightScaleApplied/visibilityApplied = false`,
   *  让界面与调用方知道"调了它也没用",避免把预留字段当成已生效的旋钮。 */
  listBranches() {
    try {
      return this.db.prepare('SELECT * FROM branch ORDER BY created_at, id').all().map((r) => ({
        ...
        // 预留标记:见上方注释(审计 A#11)。界面不要给这两个字段做"生效中"的暗示。
        weightScaleApplied: false,
  ...
  setConvBranch(source, convId, branchId) {
    ...
    // ⚠️ 存在性检查(审计 A#10,2026-09-22):覆盖层是"幽灵枝"的**写入侧** ——
    // 旧实现照单全收,指向不存在的枝也能落库;读取侧(branchCounts)只好把它对账回主干,
    // 界面则凭空多出一行"名字 = 裸 UUID"的枝。主干视为恒存在(它是隐式默认归属,
    // 历史上没有 branch 行也照样成立,不能因为迁移没跑到就拒收主干归属)。
    if (bid !== TRUNK_ID) {
      let row = null;
      try {
      ...
      if (!row) return { ok: false, reason: 'no-branch' };
    }
  ...
  reparentBranch(branchId, newParentId) {
    ...
    // ⚠️ 存在性检查(审计 A#10,2026-09-22):旧实现只查**父**,不查**自己** ——
    // 传一个不存在的枝 id 时 `UPDATE ... WHERE id=?` 影响 0 行,却照样返回 ok:true;
    // 调用方(建枝/并脉链路)以为挂上了,后续再拿这个 id 去写归属,就是孤儿制造源。
    let me = null;
    try {
      me = this.db.prepare('SELECT id FROM branch WHERE id=?').get(bid);
    } catch {
      me = null;
    }
    if (!me) return { ok: false, reason: 'no-branch' };
    if (pid !== TRUNK_ID) {
      let p = null;
      try {
  ...
  deleteBranch(branchId, { force = false } = {}) {
    ...
    let row = null;
    try {
      // 1.5.1 红蓝对抗(红队 E 顺带发现):此前只 `SELECT id`,而返回体用的是 `row.name`
      // ⇒ **回执的 name 恒为空串**,删枝留痕只能回落到裸 id。查询与消费必须同口径。
      row = this.db.prepare('SELECT id, name FROM branch WHERE id=?').get(bid);
    } catch {
      row = null;
    }
   ...
   *
   *  ⚠️ 预留字段(审计 A#11,2026-09-22 定为方案 b):写进去的 `weight_scale`
   *  **当前不参与检索打分**(L1 = raw × 血缘档位 × 矛盾系数)。保留写入与显示,
   * 是为了不破坏"复盘期可调"的界面与既有数据;但返回体里明确带
   * `applied:false` + `note`,避免调用方(与主人)误以为检索排序已经跟着变。
   * 让它真正生效属于**改变检索语义**,需主人拍板(方案 a),不在本次审计修法内。 */
  setBranchWeight(branchId, scale) {
    ...
    return {
      ok: true, id: bid, weightScale: clamped,
      applied: false, note: 'weight_scale 为预留字段:已写入并显示,当前不参与检索打分',
    };
  }
  ...
  // ---- 软遗忘(Part G G2,2026-09-25) ----
  //
  // 原则(用户 2026-09-25 定):**永远不做彻底删除**。遗忘 = 先把原数据归档落盘(原文 + 概述 +
  // 改动日志),再在库里打一个标记,召回路径据标记排除。行仍在库里、归档文件在磁盘上 ——
  // 两条独立还原路径,任何一条都能反悔。
  //
  // 为什么"真删"是错的:① 只删概述不删原文 ⇒ summarizer 下一轮就从原文重建,等于没忘;
  // ② 连原文一起删 ⇒ 一旦发现"忘错了"就再也回不来(backfill 的前提恰恰是原文还在)。
  // 所以这里只做"标记",不做 DELETE —— 删除权限留在人的手里。

  /** 已遗忘条目的键集(`source|conv_id`),供召回路径排除。 */
  forgottenConvIdSet() {
    try {
      const rows = this.db.prepare('SELECT source, conv_id FROM forgotten').all();
      return new Set(rows.map((r) => String(r.source) + '|' + String(r.conv_id)));
    } catch {
      return new Set();
    }
  }
   ...
   * 打遗忘标记。
   *
   * **调用方必须先行完成归档**(此处只写库)—— 而且**必须确认那份归档不是空的**:
   * B-05(1.5.1 红蓝对抗)的教训是"归档成功"与"备份到了东西"是两件事,
   * 对一条不存在的条目(或概述行在、原文已被硬删的条目)归档出来是 0 轮 ——
   * 文件系统上确实有个目录,判据却什么都没保住,标记一打,这条 id 就永不召回,
   * 界面还显示"忘过了"。所以这里**再兜一层**(fail-closed 的第二道):
   *   · 没给 `archivePath` ⇒ 调用方没归档就想过关;除非它显式声明
   *     `archiveTurns: 0`(它自己知道并承担),否则拒绝;
   *   · `archiveTurns` 声明了轮数(≥1)⇒ 信声明(HTTP 通道的调用方按此口径传);
   *   · 没声明 ⇒ 直接验盘(归档目录里 `turns.jsonl` 有没有内容),
   *     这样"裸调 `forgetEntry`"也过不了关 —— 默认拒绝形态。
   * ⚠️ 仍然缺的一环在**调用侧**(`tools.js` 的 forget handler):它只判 `arc.ok`,
   *    不判 `arc.empty` ⇒ 想彻底堵住 B-05 还得在那儿加一句守卫(见审计台账)。
   */
  forgetEntry({ source, convId, reason = '', archivePath = '', actor = 'user', archiveTurns = null } = {}) {
    const s = String(source || '').trim();
    const c = String(convId || '').trim();
    if (!s || !c) return { ok: false, reason: 'bad-key' };
    const dir = String(archivePath || '').trim();
    if (!dir) return { ok: false, reason: 'empty-archive', message: '没有归档路径 —— 先归档再打标记' };
    if (archiveTurns === null || archiveTurns === undefined) {
      let n = 0;
      try {
        n = existsSync(join(dir, 'turns.jsonl'))
          ? readFileSync(join(dir, 'turns.jsonl'), 'utf8').split('\n').filter((l) => l.trim()).length
          : 0;
      } catch { n = 0; }
      if (n === 0) {
        return { ok: false, reason: 'empty-archive', message: '归档里没有原文(0 轮)', archivePath: dir };
      }
    } else if (!(Number(archiveTurns) > 0)) {
      return { ok: false, reason: 'empty-archive', message: '归档里没有原文(0 轮)', archivePath: dir };
    }
    const oneLine = (x, n) => String(x == null ? '' : x).replace(/[\r\n\t]+/g, ' ').trim().slice(0, n);
    try {
      this.db.prepare(`INSERT INTO forgotten (source,conv_id,at,reason,archive_path,actor)
        VALUES (?,?,?,?,?,?) ON CONFLICT(source,conv_id) DO UPDATE SET
        at=excluded.at, reason=excluded.reason, archive_path=excluded.archive_path, actor=excluded.actor`)
        .run(s, c, utcIso(), oneLine(reason, 300), oneLine(archivePath, 400),
          String(actor || 'user').toLowerCase() === 'ling' ? 'ling' : 'user');
      return { ok: true, source: s, convId: c, archivePath: String(archivePath || '') };
    } catch (e) {
      return { ok: false, reason: 'db', message: String(e?.message ?? e).slice(0, 120) };
    }
  }

  /** 取消遗忘标记(归档文件不动 —— 它本来就是"曾经忘过"的证据)。 */
  unforgetEntry({ source, convId } = {}) {
    try {
      const r = this.db.prepare('DELETE FROM forgotten WHERE source=? AND conv_id=?')
        .run(String(source || ''), String(convId || ''));
      return { ok: true, removed: Number(r.changes || 0) };
    } catch {
      return { ok: false, reason: 'db' };
  ...
  /** 遗忘清单(最近的在前)。 */
  listForgotten({ limit = 100 } = {}) {
    try {
      const n = Math.max(1, Math.min(1000, Number(limit) || 100));
      return this.db.prepare('SELECT * FROM forgotten ORDER BY at DESC LIMIT ?').all(n).map((r) => ({
        source: String(r.source || ''), convId: String(r.conv_id || ''), at: String(r.at || ''),
        reason: String(r.reason || ''), archivePath: String(r.archive_path || ''), actor: String(r.actor || 'user'),
      }));
    } catch {
      return [];
    }
  ...
  /** 导出单条的**全部数据**(概述 + 会话元数据 + 原文),供归档落盘。 */
  exportEntry(source, convId) {
    const s = String(source || '');
    const c = String(convId || '');
    const out = { source: s, convId: c, at: utcIso(), overview: null, sessionMeta: null, turns: [] };
    try { out.overview = this.overviewById(s, c); } catch { /* 允许缺失 */ }
    try { out.sessionMeta = this.db.prepare('SELECT * FROM session_meta WHERE session_id=?').get(c) || null; } catch { /* 允许缺失 */ }
    try {
      out.turns = this.db.prepare('SELECT seq, role, ts, model, text FROM dsh_turns_raw WHERE session_id=? ORDER BY seq').all(c);
    } catch { /* 允许缺失 */ }
    // 导入源的原文带命名空间前缀(见 RAW_IMPORT_PREFIX),裸 id 取不到时按前缀再试一次
    if (!out.turns.length) {
      try {
        out.turns = this.db.prepare('SELECT seq, role, ts, model, text FROM dsh_turns_raw WHERE session_id=? ORDER BY seq')
          .all(RAW_IMPORT_PREFIX + c);
      } catch { /* ignore */ }
    }
    return out;
  }
   ...
   * 从归档写回(回灌)。
   *
   * **调用方必须先归档当前状态** —— 本方法只管写,不做任何备份。
   * 回灌的危险性高于遗忘:遗忘会留下"少了一条"的可见痕迹,而回灌是**往里加东西**,
   * 加错了很难察觉(用户 2026-09-25:「backfill 比 forget 危险多了,因为它更难以觉察」)。
   *
   * 两道保护:① 写回的概述带 `origin` 标记(默认 `backfill-v1`),界面上看得出"这条是灌回来的";
   * ② 不传 `keepUserState:false` ⇒ **不动主人留下的命中数与置顶**,回灌只补内容、不夺用户态。
   */
  restoreEntry({ source, convId, turns = [], overview = null, origin = 'backfill-v1' } = {}) {
    const s = String(source || '').trim();
    // ⚠️ B-01(1.5.1 红蓝对抗,2026-09-25):写入侧**必须**与本模块契约(`:25` "写入侧一律带前缀")
    //   和 `import-file.js:285` 同口径走 rawSessionId。
    //   此前这里直接用裸 convId ⇒ import 源的原文落成**裸 id**,而概述器按 dsh 会话扫描
    //   `dsh_turns_raw`,于是给它建出 `source='dsh'` 的镜像行 ⇒ **同 id 双源**
    //   (D1 一代 / B7 二代 之后,由全新工具 memory_backfill 打开的第三次)。
    //   对"已带前缀"的正确输入是无操作(idempotent),只拦畸形输入 —— 默认拒绝形态。
    const c = rawSessionId(s, String(convId || '').trim());
    if (!s || !c) return { ok: false, reason: 'bad-key' };
    let wroteTurns = 0;
    try {
      const ins = this.db.prepare(`INSERT INTO dsh_turns_raw (session_id,seq,role,ts,model,text)
        VALUES (?,?,?,?,?,?) ON CONFLICT(session_id,seq) DO UPDATE SET
        role=excluded.role, ts=excluded.ts, model=excluded.model, text=excluded.text`);
      this.db.exec('BEGIN');
      for (const t of (Array.isArray(turns) ? turns : [])) {
        ins.run(c, Number(t.seq) || 0, String(t.role || ''), t.ts ?? null, t.model ?? null, String(t.text ?? ''));
        wroteTurns += 1;
      }
      this.db.exec('COMMIT');
    } catch (e) {
    ...
    let wroteOverview = false;
    if (overview && typeof overview === 'object') {
      try {
        this.upsertOverview({ ...overview, source: s, conv_id: c, origin: String(origin || 'backfill-v1') });
        wroteOverview = true;
      } catch { /* 概述写失败不算整体失败:原文已回去,概述可由概述器重建 */ }
    }
    return { ok: true, source: s, convId: c, turns: wroteTurns, overview: wroteOverview };
  }
   ...
   */
  logBranch(branchId, action, { before = '', after = '', note = '', actor = 'user' } = {}) {
    ...
    // G4(2026-09-25):actor 记录**谁改的** —— 工具通道上线前器灵只能经 API/开库改树,
    //   而旧 logBranch 无此字段,91 笔器灵改动在复盘留痕里全被显示成"主人改的"。
    //   取值 'user' | 'ling'(默认 'user' 以兼容既有调用点)。
    const oneLine = (s, n) => String(s == null ? '' : s).replace(/[\r\n\t]+/g, ' ').trim().slice(0, n);
    const who = String(actor || 'user').toLowerCase() === 'ling' ? 'ling' : 'user';
    try {
      this.db.prepare(`INSERT INTO branch_log (at,branch_id,action,before_val,after_val,note,actor)
        VALUES (?,?,?,?,?,?,?)`)
        .run(utcIso(), oneLine(branchId, 64), oneLine(action, 40),
          oneLine(before, 200), oneLine(after, 200), oneLine(note, 300), who);
      return { ok: true };
    } catch {
      return { ok: false, reason: 'db' };
    }
   ...
   */
  branchLogDigest({ limit = 5, afterId = null } = {}) {
    const max = Math.max(1, Math.min(20, Number(limit) || 5));
    // B#9(2026-09-22 审计):水位必须**按会话** —— 全局键会让会话 A 消费之后、
    // 会话 B 再也看不到同一批改动。调用方传 afterId(取自 kv `branch_log_wm.<sessionId>`);
    // 未传才退回全局键(仅为兼容旧调用点,新代码不应依赖)。
    const wm = (afterId !== null && afterId !== undefined && Number.isFinite(Number(afterId)))
      ? Number(afterId)
      : Number(this.kvGet('branch_log_wm') || 0);
    let rows = [];
    try {
    ...
    // ── B1(1.5.1):真实剩余笔数**必须问库** ─────────────────────────────────────
    // 上面 listBranchLog 只取最近 120 行,积压超过 120 时窗口恒满;旧代码拿
    // `fresh.length - lines.length`(= limit 夹取差,恒为 120-5=115)当"另有 N 笔",
    // 于是主人读到的是一个与真实积压无关的**常数**(实测 live 上未读 394 笔,仍显示 115)。
    // 这里补一条独立 COUNT:branch_log.id 是 INTEGER PRIMARY KEY(AUTOINCREMENT),
    // `WHERE id > ?` 是主键范围扫描 —— 只走索引不拉行,代价与积压量无关,
    // 所以**不需要**把 limit 调大去"凑数"(那才是真把几百行读进内存)。
    // 只服务展示:水位推进仍由下方 `maxId` 负责(B3 按主人的决定推迟到 1.5.2)。
    let realTotal = fresh.length;            // 兜底:COUNT 失败时退回窗口内条数(已知下限)
    try {
      const agg = this.db.prepare('SELECT COUNT(*) AS n FROM branch_log WHERE id > ?').get(wm);
      if (agg) realTotal = Math.max(Number(agg.n) || 0, fresh.length);
    } catch { /* 库出错时退回窗口口径:摘要宁可少报,也不能整段消失 */ }
    ...
    // 显示面修正(2026-09-22 实测):原实现对一切"查不到名字的 id"回退成「(已删枝 …)」,
    // 但 branch_log 的 before/after **不只装枝 id** —— snapshot 装的是快照号(#9)、
    // assign 装的是记忆条目 id(026ebd86),二者都被误标成"已删枝"(用户截图所见:
    // `snapshot:主干 → (已删枝 #9)` 实际一根枝都没删)。先按 id 形态分流再回退查名。
    const nm = (v) => {
      const k = String(v || '');
      if (!k) return '?';
      const hit = names.get(k);
      if (hit) return hit;                                                  // 真的是现有枝
      if (/^#?\d+$/.test(k)) return '快照 ' + (k[0] === '#' ? k : '#' + k);  // 快照号
      if (/^(br|vein):/.test(k)) return '(已删枝 ' + k.slice(-8) + ')';      // 长 id 形态 ⇒ 枝
      return '条目 ' + k.slice(-8);                                         // 其余按记忆条目
    };
    ...
    const lines = fresh.slice(0, max).map((r) => {
      ...
      const line = (() => {
        switch (String(r.action || '')) {
          ...
          case 'snapshot': return '保存了' + nm(r.after) + (r.note ? '(' + r.note + ')' : '');
          case 'restore': return '用' + nm(r.after || r.before) + '恢复了一次树结构';
          case 'assign': return '把「' + nm(r.after) + '」挪到了「' + who + '」下';
          case 'delete': {
            // D-T04(1.5.1 红蓝对抗):新行 after 存**被删枝的名字**,旧行 after 存**枝 id**。
            // 此前一律用 who = nm(branchId),而旧写入端恒传 TRUNK_ID ⇒ 每行都渲染成
            // 「删掉了枝「主干」」(实测:live 上 59 笔删枝留痕全是这句),被删枝的名字全丢。
            const a = String(r.after || '');
            const label = /^(br|vein):/.test(a) ? nm(a) : (a || who);
            return '删掉了枝「' + label + '」';
          }
          case 'autobuild': return '自动建了一条枝「' + nm(r.after) + '」' + (r.note ? '(' + r.note + ')' : '');
          // ── 1.5.0 新增的动作(A-09 / F1 / C-15 / D-S03:此前全落进 default ⇒ 渲染成
          //    「主人 distill:学术与写作 → 条目 候选 #10」这类乱码,且 note 被整条丢弃) ──
          case 'distill': return '为「' + who + '」提炼了一条主脉记忆草稿' + (r.note ? '(' + r.note + ')' : '');
          case 'vein-adopt': {
            const rejected = String(r.after || '').includes('驳回');
            if (rejected) {
              // who 为 '?' 的只有 1.5.0 的**存量驳回行**(写入端当年没回传 veinId,所以 branch_id 是空串)
              // ⇒ 宁可不提那条主脉,也不要吐一个 '?' 给模型猜。
              return who === '?' ? '驳回了一条提炼候选(什么都没改)' : '驳回了「' + who + '」的一条提炼候选(什么都没改)';
            }
            // A-01:覆盖是**预期行为**(概括随主脉一起长),但注入面要说清"旧的没丢,已归档"。
            return String(r.after || '').includes('覆盖')
              ? '采纳了一条提炼候选,作为「' + who + '」的新版主脉记忆(旧版已归档)'
              : '采纳了一条提炼候选,作为「' + who + '」的主脉记忆';
          }
          case 'theme-rebuild': return '重建了主题结构:' + (r.before || '') + ' → ' + (r.after || '') + (r.note ? '(' + r.note + ')' : '');
          // ── 器灵工具通道会写的动作(1.5.1 一并补上,免得将来重演同一处乱码) ──
          case 'create': return '建了一条枝「' + (r.after || who) + '」' + (r.note ? '(' + r.note + ')' : '');
          case 'forget': return '把' + nm(r.after) + '标记为遗忘' + (r.note ? '(' + r.note + ')' : '');
          case 'unforget': return '撤销了' + nm(r.after) + '的遗忘标记';
          case 'backfill': return '回灌了' + nm(r.after) + (r.note ? '(' + r.note + ')' : '');
          default: return (r.action || '操作') + ':' + who + (r.after ? ' → ' + nm(r.after) : '');
        }
      })();
      // G4(2026-09-25):每行带主语 —— 器灵经工具通道改的树记 'ling',
      // 不再和"主人改的"混在同一批留痕里(Part G 之前 91 笔器灵改动全记在主人名下)。
      // 中文动作短语(把「」…)直接接主语;英文动作名(distill:/gather:)前面补空格,免得拼成"主人distill:"。
      const subject = String(r.actor || 'user') === 'ling' ? '器灵' : '主人';
      return flat(subject + (/^[A-Za-z0-9]/.test(line) ? ' ' : '') + line);
    });
    // 最多 max 行 + 一行计数。折算的**余量走 realTotal**(B1:真实剩余笔数)。
    if (realTotal > lines.length) lines.push('…另有 ' + (realTotal - lines.length) + ' 笔改动');
    return { lines, maxId: Number(fresh[0].id), total: realTotal };
  }
  ...
  /** 尚未同步给器灵的改动笔数(复盘守门,2026-09-22 用户:「动记忆树不能是随意的」)。
   *
   *  判据:自器灵**上一次开口**(注入推进水位)以来累积了多少笔结构改动。
   *  水位取 `branch_log_wm.last`(由 `inject.js` 的 advanceLogWatermark 在投递时写入),
   *  因此它与"按会话水位"互补:会话水位决定**该会话**看到什么,本键决定**是否还没交代**。
   */
  branchLogPending() {
    let wm = 0;
    let hasKey = false;
    try {
      const raw = this.kvGet('branch_log_wm.last');
    ...
    let maxId = 0;
    try {
      // 'snapshot'(保存快照)不计入待交代笔数(2026-09-25 用户:「单纯保存不弹修改」)。
      // 依据:保存快照**不改树**——它只是把当前结构另存一份,是"大改之前的保险",
      // 不是"改动"。把它算成债会让主人每存一次就看到「⚠ 有 N 笔改动还没跟器灵说过」,
      // 而实际上树一个字节没变。留痕仍写进 branch_log(审计价值保留),只是不推进债。
      // 注:restore(恢复)/reparent/rename 等**真改结构**的动作照旧计入。
      // maxId 仍取全表最大(含 snapshot),否则水位会卡在 snapshot 行之前反复重算。
      const r = this.db.prepare(
        'SELECT COALESCE(MAX(id),0) AS m,'
        + " SUM(CASE WHEN action <> 'snapshot' THEN 1 ELSE 0 END) AS n"
        + ' FROM branch_log WHERE id > ?'
      ).get(wm);
      maxId = Number((r && r.m) || 0);
      pending = Number((r && r.n) || 0);
    } catch {
      /* noop */
    }
    // 升级后首次运行:库里可能已积压上百条历史留痕(1.4.0 之前没有这个键),
    // 那些属于"机制上线前的事",不该一次性报成未交代 —— 用当前最大 id 作起点并落键,
    // 避免"一升级就显示 118 笔未同步"的假警报。此后水位只由 inject 的投递推进。
    if (!hasKey && maxId > 0) {
      try {
        this.kvSet('branch_log_wm.last', String(maxId));
      } catch {
        /* noop */
      }
      return { watermark: maxId, maxId, pending: 0, initialized: true };
    }
    return { watermark: wm, maxId, pending };
  }

  /** 整棵树(UI 用):嵌套 children + 每枝会话数 + 横向连边 + **层级(计算属性,不落库)**。
   *
   *  ⚠️ 两个计数**语义不同**(审计 A#6,2026-09-22),别混用:
   *    · `sessions` = `session_meta` **台账行数**(含空壳会话)—— "这个枝登记了多少会话",
   *      存量消费方依赖它,故保持原样(空壳也是登记过的会话,只是没有内容);
   *    · `members`  = **展开实际会给出的条数**,与 `branchMembers()` 同一谓词(有标题 OR 有原文)。
   *  界面里挨着 ▶ 的数字**必须用 `members`**:台账口径里那批空壳永远展不开,
   *  实测主干写「76 个会话」而展开只有 15 条 —— 61 条是点不开的假广告。
   *  两个数都给,是为了让"为什么对不上"在返回体里一眼可查,而不是靠猜。
   */
  branchTree() {
    ...
    let expand = new Map();
    try {
      expand = this.branchMemberCounts().members;
    } catch {
      /* noop */
    }
    const byId = new Map();
    for (const n of nodes) {
      byId.set(n.id, {
        ...n,
        sessions: counts.get(n.id) || 0, // 台账(含空壳)—— 兼容既有消费方
        members: expand.get(n.id) || 0, // 展开实际条数 —— 界面计数用这个
        children: [],
      });
    }
    const roots = [];
  ...
  /** 存一份当前树结构的快照。
   *  `kind`:'manual'(默认,主人手动保存) | 'auto'(恢复前的自动备份)。
   *  审计 A#9:两者外表完全一致,必须靠这一列区分 —— 自动备份是"撤回上次恢复"的唯一路径,
   *  手动快照可以随便删,自动备份不能删到一份不剩(见 deleteTreeSnapshot)。 */
  snapshotTree({ name = '', note = '', kind = 'manual' } = {}) {
    const now = utcIso();
    ...
    try {
      ...
      const nm = String(name || '').trim() || (k === 'auto' ? '自动快照 - ' : '手动快照 - ') + localStamp();
      const r = this.db.prepare('INSERT INTO tree_snapshot (at,name,note,kind,data) VALUES (?,?,?,?,?)')
        .run(now, nm.slice(0, 80), String(note || '').slice(0, 300), k, JSON.stringify(data));
      return {
        ok: true,
  ...
  listTreeSnapshots({ limit = 50 } = {}) {
    ...
    try {
      return this.db.prepare('SELECT id, at, name, note, kind, LENGTH(data) AS bytes FROM tree_snapshot ORDER BY id DESC LIMIT ?')
        .all(n)
        .map((r) => ({
          id: Number(r.id),
          ...
          name: String(r.name || ''),
          note: String(r.note || ''),
          // 审计 A#9:列表必须带 kind,界面才分得出"哪个是自动备份、删不得"
  ...
  restoreTree(snapshotId) {
    ...
    const backup = this.snapshotTree({ kind: 'auto', name: '自动快照 - ' + localStamp(), note: '恢复快照 #' + sid + ' 之前的自动备份' });
    let detached = 0;
    try {
  ...
  /**
   * 删一份树快照。审计 A#9(2026-09-22):**拒绝删掉最后一份 `kind='auto'` 的快照**。
   *
   * `restoreTree` 在动手之前自动存一份备份 —— 那是"撤回刚才那次恢复"的唯一路径。
   * 而在这一列存在之前,自动备份与手动快照在库里、界面上**外表完全一致**(名字都只是"… - 时间"),
   * 主人顺手清理旧快照时完全可能把唯一的退路删掉,而"误恢复"之后是没有第二次机会的。
   *
   * 存在性检查同批补上(与 resolveConflict 同一条纪律):删一个不存在的 id 不该报 ok。
   * `force: true` 是留给维护动作的显式逃生口(默认关闭)—— 但**没有它就无法删除过期的自动备份**,
   * 所以调用方需要真删时应当显式传入,而不是把保护拆掉。
   */
  deleteTreeSnapshot(snapshotId, { force = false } = {}) {
    const sid = Number(snapshotId);
    if (!Number.isFinite(sid)) return { ok: false, reason: 'bad-id' };
    try {
      const row = this.db.prepare('SELECT id, kind FROM tree_snapshot WHERE id=?').get(sid);
      if (!row) return { ok: false, reason: 'not-found' };
      const kind = String(row.kind || 'manual');
      if (!force && kind === 'auto') {
        const auto = this.db.prepare("SELECT COUNT(*) AS n FROM tree_snapshot WHERE kind='auto'").get();
        if (Number(auto?.n || 0) <= 1) {
          return {
            ok: false,
            reason: 'last-auto',
            kind,
            message: '这是最后一份自动备份(撤回上次树恢复的唯一路径),不能删除',
          };
        }
      }
      const r = this.db.prepare('DELETE FROM tree_snapshot WHERE id=?').run(sid);
      return { ok: true, removed: Number(r.changes || 0), id: sid, kind };
    } catch {
      return { ok: false, reason: 'db' };
    }
  }
   ...
   *
   * 计数口径见 `branchMemberCounts()`:同一谓词算出的条数必须等于本函数返回的长度。
   */
  branchMembers(branchId, { limit = 300 } = {}) {
    const bid = String(branchId || TRUNK_ID);
    // ⚠️ 审计 A#5(2026-09-22):`limit` 是**两次查询共用的总预算**,不是每边各一份 ——
    // 旧实现两次 `.all(bid, n)` ⇒ 实际最多吐 2×limit(limit=300 时 600 条);
    // 且 `Number(limit) || 300` 把 `limit=0` 吞成默认值 ⇒ "要 0 条"永远拿不到空数组。
    // 现在:0 就是空;非 0 时先取会话,覆盖层条目只吃剩下的预算。
    const asked = Number(limit);
    const n = Number.isFinite(asked) ? Math.max(0, Math.min(2000, Math.trunc(asked))) : 300;
    if (n === 0) return [];
    ...
    try {
      for (const r of this.db.prepare(`SELECT m.session_id AS cid,
        ...
        WHERE m.branch_id=? AND ${MEMBER_LIVE_SESSION}
    ...
    const left = n - out.length; // 剩余预算(会话先取,条目吃剩下的)
    try {
      ...
      for (const r of this.db.prepare(`SELECT c.source AS src, c.conv_id AS cid,
        ...
        WHERE c.branch_id=? AND ${MEMBER_LIVE_ENTRY}
        ORDER BY c.conv_id LIMIT ?`).all(bid, left)) {
  ...
  /**
   * 各枝"可展开成员"计数(审计 A#6,2026-09-22)—— 与 `branchMembers()` **同一谓词**的 per-branch 计数。
   *
   * 界面上的「N 个会话」应当用它,而不是 `session_meta` 的台账行数:台账里躺着大量**空壳会话**
   * (既无概述标题、又没有任何原文轮次),它们永远展不开,计进去就是假广告。
   *
   * 返回三个**双读法**视图(见 `countView()`:下标 `c[id]` 与 `.get(id)` 都可用):
   *   `members` —— 展开实际条数(会话 + 覆盖层条目),`branchMembers()` 不截断时长度即此值;
   *   `sessions` —— 其中会话部分(session_meta 来源);
   *   `rows`     —— session_meta 原始行数(含空壳;只给排障/观测,**界面不要拿它当计数**);
   *   `byMembers` —— 与 `members` 同一份数据(历史命名,保留以避免对接方改名)。
   */
  branchMemberCounts() {
    ...
    try {
      for (const r of this.db.prepare(`SELECT m.branch_id AS bid,
        CASE WHEN ${MEMBER_LIVE_SESSION} THEN 1 ELSE 0 END AS live
        FROM session_meta m LEFT JOIN conv_overview o ON o.source='dsh' AND o.conv_id=m.session_id`).all()) {
        const bid = String(r.bid || TRUNK_ID);
        bump(rows, bid);
        if (Number(r.live) === 1) {
          bump(members, bid);
          bump(sessions, bid);
    ...
    try {
      for (const r of this.db.prepare(`SELECT c.branch_id AS bid,
        CASE WHEN ${MEMBER_LIVE_ENTRY} THEN 1 ELSE 0 END AS live
        FROM conv_branch c LEFT JOIN conv_overview o ON o.source=c.source AND o.conv_id=c.conv_id`).all()) {
        if (Number(r.live) === 1) bump(members, String(r.bid || TRUNK_ID));
      }
    } catch { /* noop */ }
    const view = countView(members);
    return { members: view, byMembers: view, sessions: countView(sessions), rows: countView(rows) };
  }
  ...
  resolveConflict(id, { winner = null, status = 'confirmed' } = {}) {
    ...
    try {
      // ⚠️ 存在性检查(审计 A#8,2026-09-22):旧实现对不存在的 id 也返回 ok:true ——
      // `UPDATE ... WHERE id=?` 影响 0 行却报成功,调用方(含"归并时顺带驳回"的批处理)
      // 会以为裁定已落地,而待复盘队列里那条其实一动没动,错误就此静默。
      const row = this.db.prepare('SELECT id FROM memory_conflict WHERE id=?').get(cid);
      if (!row) return { ok: false, reason: 'not-found' };
   ...
   */
  // ---- 主脉提炼(R7,2026-09-23):子枝概述 → 器灵提炼 → 审核队列 ----
  // 设计稿 §5:主脉装的内容**不是任何子枝的内容**,而是"从多条枝里看出来的共性";
  // 三条硬约束 —— 走审核(不直接进记忆) / 标注派生来源 / 与并脉解耦。

  /** 提炼素材:主脉及其子枝下所有**带摘要**的记忆(只要概述,不碰原文)。 */
  veinMaterial(veinId, { limit = 24, chars = 300 } = {}) {
    const bid = String(veinId || '');
    if (!bid) return [];
    const n = Math.max(1, Math.min(60, Math.trunc(Number(limit) || 24)));
    const cap = Math.max(80, Math.min(1200, Math.trunc(Number(chars) || 300)));
    const ids = [bid];
    try {
      for (const r of this.db.prepare('SELECT id FROM branch WHERE parent_id=?').all(bid)) ids.push(String(r.id));
    } catch { /* noop */ }
    const marks = ids.map(() => '?').join(',');
    const out = [];
    const push = (src, cid, title, summary) => {
      const s = String(summary || '').replace(/\s+/g, ' ').trim();
      if (!s) return;
      out.push({
        source: String(src), convId: String(cid),
        title: String(title || '').slice(0, 60), summary: s.slice(0, cap),
      });
    };
    // ⚠️ A-02(1.5.1 红蓝对抗,2026-09-25):提炼素材**必须排除已遗忘的记忆**。
    //   实测全库只有 `l1.js` 的 selectL1 消费了 `forgotten` —— 而本函数是**会写回记忆**的路径:
    //   已遗忘的记忆被当素材 → 提炼 → 采纳 → 重新写回 conv_overview ⇒ **遗忘失效**。
    //   与 l1 同口径按 (source, conv_id) 排除;两条 SQL 都要加,漏一条等于没加。
    try {
      for (const r of this.db.prepare(`SELECT o.source AS src, o.conv_id AS cid, o.title, o.summary
        FROM session_meta m JOIN conv_overview o ON o.source='dsh' AND o.conv_id=m.session_id
        WHERE m.branch_id IN (${marks}) AND TRIM(COALESCE(o.summary,'')) <> ''
          AND NOT EXISTS (SELECT 1 FROM forgotten f WHERE f.source=o.source AND f.conv_id=o.conv_id)
        ORDER BY o.hit_count DESC, o.updated_at DESC LIMIT ?`).all(...ids, n)) {
        push(r.src, r.cid, r.title, r.summary);
      }
    } catch { /* noop */ }
    const left = n - out.length;
    try {
      if (left > 0) {
        for (const r of this.db.prepare(`SELECT c.source AS src, c.conv_id AS cid, o.title, o.summary
          FROM conv_branch c JOIN conv_overview o ON o.source=c.source AND o.conv_id=c.conv_id
          WHERE c.branch_id IN (${marks}) AND TRIM(COALESCE(o.summary,'')) <> ''
            AND NOT EXISTS (SELECT 1 FROM forgotten f WHERE f.source=o.source AND f.conv_id=o.conv_id)
          ORDER BY o.hit_count DESC, o.updated_at DESC LIMIT ?`).all(...ids, left)) {
          push(r.src, r.cid, r.title, r.summary);
        }
      }
    } catch { /* noop */ }
  ...
  /** 记一条提炼候选(审核队列;**不**直接进记忆)。 */
  addVeinSuggestion({ veinId, text, sources = [], model = '', note = '' } = {}) {
    const bid = String(veinId || '');
    const t = String(text || '').replace(/\s+/g, ' ').trim().slice(0, 1200);
    if (!bid || !t) return { ok: false, reason: 'bad-arg' };
    const now = utcIso();
    try {
      const r = this.db.prepare(`INSERT INTO vein_suggestions (at,vein_id,text,sources,model,status,note)
        VALUES (?,?,?,?,?,'new',?)`).run(
        now, bid, t,
        JSON.stringify(Array.isArray(sources) ? sources.slice(0, 40) : []),
        String(model || '').slice(0, 80), String(note || '').slice(0, 300),
      );
      return { ok: true, id: Number(r.lastInsertRowid), at: now };
    } catch (e) {
      return { ok: false, reason: 'db', error: String(e?.message ?? e).slice(0, 160) };
    }
  }

  listVeinSuggestions({ status = 'new', veinId = '', limit = 50 } = {}) {
    const n = Math.max(1, Math.min(200, Math.trunc(Number(limit) || 50)));
    const where = [];
    const args = [];
    if (status) { where.push('status=?'); args.push(String(status)); }
    if (veinId) { where.push('vein_id=?'); args.push(String(veinId)); }
    try {
      return this.db.prepare(`SELECT * FROM vein_suggestions
        ${where.length ? 'WHERE ' + where.join(' AND ') : ''}
        ORDER BY id DESC LIMIT ?`).all(...args, n).map((r) => ({
        id: Number(r.id),
        at: r.at ? String(r.at) : '',
        veinId: String(r.vein_id || ''),
        text: String(r.text || ''),
        sources: (() => { try { return JSON.parse(String(r.sources || '[]')); } catch { return []; } })(),
  ...
  /** 裁定:采纳 → 写入**主脉自己的记忆**(复用 conv_overview,设计稿 §2.3)。 */
  resolveVeinSuggestion(id, { action = '', text = '', note = '', actor = 'user' } = {}) {
    const sid = Number(id);
    if (!Number.isFinite(sid)) return { ok: false, reason: 'bad-id' };
    let row = null;
    try { row = this.db.prepare('SELECT * FROM vein_suggestions WHERE id=?').get(sid); } catch { row = null; }
    if (!row) return { ok: false, reason: 'not-found' };
    // A-05(1.5.1 红蓝对抗):动作**白名单化 + 默认拒绝**。此前的判据是"**不等于** 'accept' 就当驳回"
    //   ⇒ `Accept` / `adopt` / 拼错 / **字段整个缺失**,全部**静默走驳回**且回 `ok:true`(实测),
    //   调用方以为成功、实际做了另一件事(而调用方是 UI,`body.action` 缺失时旧代码反而会**采纳**)。
    //   改成:不认识的动作**一个字节都不动库**,并如实回报期望值。
    const act = String(action || '').trim().toLowerCase();
    if (act !== 'accept' && act !== 'reject') return { ok: false, reason: 'bad-action', want: 'accept|reject' };
    if (act === 'reject') {
      try {
        this.db.prepare("UPDATE vein_suggestions SET status='rejected', note=?, resolved_at=? WHERE id=?")
          .run(String(note || '').slice(0, 300), utcIso(), sid);
        // 1.5.1 红蓝对抗(A-09):驳回也必须回传 veinId —— 此前只回 {ok,id,status},
        // 调用方(api.js 的留痕)取 r.veinId 拿到 undefined ⇒ branch_id 落成空串 ⇒
        // 注入面渲染出「主人 vein-adopt:? → 条目 已驳回」(实测 10/10 笔驳回都是这样)。
        return { ok: true, id: sid, status: 'rejected', veinId: String(row.vein_id || '') };
      } catch { return { ok: false, reason: 'db' }; }
    }
    const body = String(text || row.text || '').replace(/\s+/g, ' ').trim().slice(0, 1200);
    if (!body) return { ok: false, reason: 'empty-text' };
    const veinId = String(row.vein_id || '');
    if (!veinId) return { ok: false, reason: 'no-vein' };
    // ── A-01(1.5.1 红蓝对抗;用户 2026-09-25 裁决「备份并覆盖」)──────────────────
    // 采纳写入的键是 `(source='vein', conv_id=<主脉 id>)` ⇒ **同一主脉的两条候选先后采纳
    // 会命中同一个 ON CONFLICT,后一条静默吃掉前一条**(无留痕、无归档 —— 数据真的没了;
    // 红队 A 与红队 E 各自独立复现)。
    // 用户的裁决与理由:主脉记忆是一种**概括**,它理应随主脉本身一起成长 ⇒ **覆盖**是对的;
    // 但覆盖必须能回头 ⇒ 动手前把旧版**归档落盘**,归档失败就**拒绝覆盖**
    // (与 forget 那条"归档失败绝不打标记"同一条纪律),并把结果回传调用方去记总变动日志。
    let replaced = null;
    try {
      const prev = this.overviewById('vein', veinId);
      if (prev) {
        const prevChars = String(prev.summary || '').length;
        const a = archiveBefore(this, {
          source: 'vein', convId: veinId, kind: 'vein-overwrite', actor,
          reason: String(note || '').trim() || ('主脉再次采纳,覆盖上一版(' + prevChars + ' 字)'),
        });
        if (!a.ok) return { ok: false, reason: 'archive-failed', message: a.message };
        replaced = { dir: a.dir, prevChars };
      }
    } catch (e) {
      return { ok: false, reason: 'archive-failed', message: String(e?.message ?? e).slice(0, 160) };
    }
    try {
      // title 走启发式截断(与 D2 一致口径),summary 存全文 —— upsertOverview 默认保护用户态。
      this.upsertOverview({
        source: 'vein', conv_id: veinId,
      ...
      // ★ 2026-09-25 修:采纳的记忆必须挂到**它所属的主脉**下。
      // 此前只写 conv_overview、不写归属 ⇒ 按枝过滤的解析链
      // (conv_branch 显式覆盖 > session_meta 会话推导 > 主干)对 vein 源一路落到**主干**,
      // 于是主人刚采纳完,在记忆中心/记忆树里按主脉看却找不到它。
      // 实测:2026-09-24 夜采纳「学术与写作」的提炼,库里 written=1 而树上不可见。
      // 挂枝失败不让整次采纳失败(记忆已经写进去了)。
      try { this.assignConv('vein', veinId, veinId); } catch (e) {
        console.warn('[dsh-ling] 主脉记忆挂枝失败:', String(e?.message ?? e).slice(0, 120));
      }
      this.db.prepare("UPDATE vein_suggestions SET status='accepted', text=?, note=?, resolved_at=? WHERE id=?")
        .run(body, String(note || '').slice(0, 300), utcIso(), sid);
      return { ok: true, id: sid, status: 'accepted', veinId, chars: body.length, branchId: veinId, replaced };
    } catch (e) {
      return { ok: false, reason: 'db', error: String(e?.message ?? e).slice(0, 160) };
    }
  }
  ...
  upsertOverview(row) {
    ...
    const userStateSet = r.keepUserState === false
      ...
      : '';
    // ── B-02(1.5.1 红蓝对抗):定名锁**必须能被写回**,否则回灌不是无损往返 ──────────────
    // 此前 INSERT 列清单与 ON CONFLICT SET 里**都没有** title_locked / title_by:
    // 归档(exportEntry 经 mapRow 取的是全列,含这一对)读得回来,却写不回去 ⇒
    // 「主人锁过的标题」这一**事实**在"遗忘 → 回灌"之后静默消失,而标题文本还在
    // (旧 SET 里那句 `title=CASE WHEN title_locked=1 …` 只是**读锁**的写法,救不了已经丢掉的锁)。
    // 写入口径取**显式给值才覆盖**(对象里带 `title_locked` 键才算"调用方要声明这把锁"):
    // ① 回灌/导入/快照恢复这些"还原一条既有行"的路径经 archive 的 overview.json 带全字段;
    // ② 概述器/批量重命名这些"只更新内容"的路径不带这对字段 ⇒ 不可能顺手解锁一条已锁的标题
    //    (`retitleCandidates` 只挑 `title_locked=0`,解锁只能走 `unlockTitle`)。
    // 取 `Number(...) ? 1 : 0` 是因为 overview.json 里是布尔(`mapRow` 把列映射成 `!!r.title_locked`)。
    const hasLock = Object.prototype.hasOwnProperty.call(r, 'title_locked');
    const lockCols = hasLock ? ', title_locked, title_by' : '';
    const lockPh = hasLock ? ',?,?' : '';
    const lockSet = hasLock ? `
         title_locked=excluded.title_locked, title_by=excluded.title_by,` : '';
    this.db.prepare(
      `INSERT INTO conv_overview
        (conv_id, source, title, started_at, updated_at, domain_tags, category, keywords, summary,
         heat, importance, last_hit_at, hit_count, overview_ok, origin${lockCols})
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?${lockPh})
       ON CONFLICT(source, conv_id) DO UPDATE SET
         ...
         summary=excluded.summary,${userStateSet}${lockSet}
      ...
      ...(hasLock ? [Number(r.title_locked) ? 1 : 0, String(r.title_by ?? '')] : []),
    );
  }
