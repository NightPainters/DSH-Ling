...
const SCHEMA_VERSION = 10; // v10:tree_snapshot.kind('manual'|'auto',审计 A#9 —— 自动备份与手动快照此前外表完全一致,删错了就没有撤回路径);v9:tree_snapshot(树的保存/恢复 R1);v8:conv_branch(非会话源的枝归属覆盖层,方案 A);v7:branch_log(复盘留痕);v6:vein_link + memory_conflict(D9-b 树的生长);v5:branch(记忆分枝 D9-a);v4:title_by(定名者:user/ai);v3:title_locked;v2:conv_overview.source 放开 CHECK
...
export class MemoryStore {
  ...
  branchLogPending() {
    ...
    let maxId = 0;
    try {
      const r = this.db.prepare('SELECT COALESCE(MAX(id),0) AS m, COUNT(*) AS n FROM branch_log WHERE id > ?').get(wm);
