...
export async function llmOnce(ctx, { system, text, maxTokens = DEEP_CFG.maxTokens, signal, effort = DEEP_CFG.effort } = {}) {
  const llm = svc(ctx, 'llm');
  if (!llm || typeof llm.stream !== 'function') throw new Error('llm service unavailable');
  const chunks = llm.stream({
    provider: DEEP_CFG.provider,
    model: DEEP_CFG.model,
...
/** DB 原文转录(role user/assistant,按 seq)。 */
export function buildTranscript(memory, sessionId, { cap = DEEP_CFG.inputCap } = {}) {
...
export async function deepSummarizeOne(ctx, memory, candidate) {
  ...
  if (candidate.file) {
    ...
    tr = buildTranscript(memory, sessionId);
  }
...
export function candidateFor(memory, sessionId) {
  const sid = String(sessionId);
  const c = memory.db.prepare("SELECT COUNT(*) n FROM dsh_turns_raw WHERE session_id=? AND role='user'").get(sid);
...
export function importRawTargets(memory, { limit = 50, excludeProcessed = false } = {}) {
  ...
  const all = memory.db.prepare(
    `SELECT t.session_id, SUM(LENGTH(t.text)) chars
       FROM dsh_turns_raw t
       JOIN conv_overview o ON o.source='import' AND o.conv_id = t.session_id
      GROUP BY t.session_id ORDER BY chars DESC`,
  ).all();
  const out = [];
  for (const r of all) {
