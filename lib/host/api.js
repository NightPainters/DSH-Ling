...
export function registerApi(ctx, deps, config) {
  ...
  const mountRoutes = (server) => {
    ...
    // 加软上限:到顶时先删**最旧的一条**(列表接口按时间倒序,末位即最旧)。
    ...
    disposers.push(register('/tree/snapshot', async (req, res) => {
      ...
      try {
        ...
        if (all.length >= MAX_TREE_SNAPSHOTS && all[all.length - 1]?.id) {
          memory.deleteTreeSnapshot(all[all.length - 1].id);
        }
      } catch { /* 上限清理失败不该挡住保存 */ }
    ...
    disposers.push(register('/branch/assign', async (req, res) => {
      const body = JSON.parse((await readBody(req)) || '{}');
      // ⚠️ 审计 B#11:此前接受**任意** branchId ⇒ "幽灵行"真落库(覆盖层里指向一条不存在的枝),
      // 表现为界面出现"名字 = 裸 UUID"的枝。写入侧先挡一道(读取侧另在 branchCounts 对账)。
      const bid = String(body.branchId || TRUNK_ID);
      if (bid !== TRUNK_ID && !memory.listBranches().some((b) => b.id === bid)) {
        return sendJson(res, 400, { ok: false, reason: 'no-branch' });
      }
      const r = memory.assignConv(body.source, body.convId, bid);
      if (r.ok) {
