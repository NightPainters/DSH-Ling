 ...
 */
const mapBranchList = (list) => new Map((Array.isArray(list) ? list : []).map((b) => [String(b.id), b]));
...
function convExistsIn(memory, source, convId) {
...
const BATCH_MS = { judge: 120000, name: 300000 };
...
export function registerApi(ctx, deps, config) {
  ...
  const mountRoutes = (server) => {
    ...
    disposers.push(register('/branches/delete', async (req, res) => {
      const body = JSON.parse((await readBody(req)) || '{}');
      const r = memory.deleteBranch(String(body.id || ''), { force: body.force === true });
      if (r.ok) memory.logBranch(TRUNK_ID, 'delete', { after: String(r.name || body.id || ''), note: '删枝' + (body.force ? '(force)' : '') });
    ...
    disposers.push(register('/branches/reparent', async (req, res) => {
      ...
      if (!beforeRow) return sendJson(res, 400, { ok: false, reason: 'no-match' });
    ...
    disposers.push(register('/branches/rename', async (req, res) => {
      ...
      if (!beforeRow) return sendJson(res, 400, { ok: false, reason: 'no-match' }); // 审计 B#3
    ...
    disposers.push(register('/veins/unlink', async (req, res) => {
      ...
      if (r.ok && !(Number(r.removed) > 0)) {
        return sendJson(res, 400, { ok: false, reason: 'no-match', removed: 0 });
      }
    ...
    disposers.push(register('/tree/snapshot', async (req, res) => {
      ...
      try {
        ...
        if (all.length >= MAX_TREE_SNAPSHOTS) {
    ...
    disposers.push(register('/branch/assign', async (req, res) => {
      ...
      const bid = String(body.branchId || TRUNK_ID);
      if (bid !== TRUNK_ID && !mapBranchList(memory.listBranches()).has(bid)) {
        return sendJson(res, 400, { ok: false, reason: 'no-branch' });
      }
      const src = String(body.source || '');
      const cid = String(body.convId || '');
      if (!src || !cid) return sendJson(res, 400, { ok: false, reason: 'bad-arg' });
      if (!convExistsIn(memory, src, cid)) {
        return sendJson(res, 400, { ok: false, reason: 'no-conv', hint: '该条目不在 conv_overview / session_meta 里:拒绝落幽灵行' });
      }
