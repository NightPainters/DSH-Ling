...
export function registerApi(ctx, deps, config) {
  ...
  const mountRoutes = (server) => {
    ...
    disposers.push(register('/vein/distill', async (req, res) => {
      ...
      if (!distilled) {
        sendJson(res, 200, { ok: false, reason: 'llm-failed', error: lastErr || 'empty', attempts, ms: Date.now() - t0 });
        return;
      }
      ...
      sendJson(res, 200, {
        ...
        materials: mat.length, ms: Date.now() - t0, reason: saved.reason, attempts,
      });
    }));
    ...
    disposers.push(register('/vein/suggestion/resolve', async (req, res) => {
      const body = JSON.parse((await readBody(req)) || '{}');
      const r = memory.resolveVeinSuggestion(Number(body.id), { action: body.action, text: body.text, note: body.note });
      if (r.ok) {
        memory.logBranch(String(r.veinId || ''), 'vein-adopt', { after: r.status === 'accepted' ? '已采纳' : '已驳回', note: '主脉提炼候选' });
      }
      sendJson(res, 200, r);
    }));
