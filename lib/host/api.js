...
import { applyModeToSession, currentMode, MODE_LABEL, resolveModeConfig } from './mode.js';
...
const AUTH_COOKIE_RE = /(?:^|;\s*)dsh-auth-[^=]+=/;
...
export function registerApi(ctx, deps, config) {
  ...
  const mountRoutes = (server) => {
    ...
    const guard = (handler) => async (req, res) => {
      if (!AUTH_COOKIE_RE.test(String(req.headers.cookie ?? ''))) {
        ...
        memory: {
          ...
          l1: l1 ? {
            items: l1.items.map((i) => ({
              conv_id: i.conv_id, title: i.title, date: (i.updated_at || i.started_at || '').slice(0, 10),
    ...
    disposers.push(register('/mode/toggle', async (req, res) => {
      ...
      if (!sessionId) {
        ...
        // ① 平台默认模型(agent-default-model)→ 决定新会话的真实推理档位
        // ② 自有 lastMode(D2 跟随源)
        ...
        const cfg = resolveModeConfig(settings.get(), next);
        let warning;
        const adm = svc(ctx, 'agentDefaultModel');
        if (adm && typeof adm.saveSelection === 'function') {
          try {
            await adm.saveSelection({ provider: cfg.provider, model: cfg.model, reasoningEffort: cfg.reasoningEffort });
          } catch (e) {
            warning = 'default-model sync failed: ' + String(e?.message ?? e);
            console.debug('[dsh-ling] saveSelection warning', warning);
          }
        } else {
          warning = 'agentDefaultModel unavailable; new-session model not synced';
    ...
    // POST /persona/hint-adopt { text }  诞生建议(语气建议/相处观察)采纳为惯例:
    //   属"成长回路"同权(与点踩采纳一致,定型下也可自由追加),去重。
    disposers.push(register('/persona/hint-adopt', async (req, res) => {
      ...
      const cur = settings.get();
      const rules = Array.isArray(cur.persona?.hardRules) ? cur.persona.hardRules : [];
      if (rules.includes(text)) {
        return sendJson(res, 200, { ok: true, exists: true, hardRules: rules.length, message: '这条已在惯例里了' });
      }
      const histKey = await archivePersona(cur, 'result');
      const s2 = await settings.update({ persona: { hardRules: [...rules, text] } });
      const affected = allSnapshotSessionIds(gate).map((sid) => ({
        sessionId: sid,
        result: invalidateSession(gate, memory, settings, sid),
      }));
      sendJson(res, 200, {
        ok: true, affected, histKey, hardRules: (s2.persona?.hardRules || []).length,
    ...
    disposers.push(register('/persona/tone-advice', async (_req, res) => {
      try {
        const workSide = sampleToneRows(memory, { side: 'work', limit: 40 });
        const lifeSide = sampleToneRows(memory, { side: 'life', limit: 40 });
        const srcWork = buildGenesisSource(workSide.rows, { cap: 9000 });
        const srcLife = buildGenesisSource(lifeSide.rows, { cap: 9000 });
        if (!srcWork && !srcLife) {
          return sendJson(res, 200, { ok: false, reason: 'empty', message: '记忆库还是空的——先接入历史,她才有可观察的素材。' });
        }
        const material = '【工作侧材料(知识/技术为主)】\n' + (srcWork || '(无)') +
          '\n\n【生活侧材料(情感/日常为主)】\n' + (srcLife || '(无)');
        const { text } = await llmOnce(ctx, {
          system: TONE_ADVICE_SYS,
          text: material,
          maxTokens: 1000,
          effort: 'low',
        });
    ...
    disposers.push(register('/persona/draft', async (_req, res) => {
      ...
      parts.push('语气基调:' + toneName + ' | 语言:' + (p.language === 'zh' ? '中文' : p.language === 'en' ? 'English' : '跟随用户'));
      if (Array.isArray(p.bottomLines) && p.bottomLines.length) parts.push('底线:\n' + p.bottomLines.map((x) => '- ' + x).join('\n'));
      if (Array.isArray(p.hardRules) && p.hardRules.length) parts.push('惯例:\n' + p.hardRules.map((x) => '- ' + x).join('\n'));
    ...
    disposers.push(register('/persona/genesis', async (req, res) => {
      ...
      const comp = composeGenesisRows(memory, { scope });
      const sourceText = buildGenesisSource(comp.rows);
      if (!sourceText) {
        return sendJson(res, 200, { ok: false, reason: 'empty', message: '还没有可读的历史——先在「接入历史」把一段对话请进来,她才能开始认识自己。' });
      }
      try {
        const { text } = await llmOnce(ctx, {
          system: GENESIS_SYS,
          text: `请阅读以下"她与用户的共同历史材料"(${scope === 'all' ? '来自全部记忆,已按生活/情感优先抽样 ' + comp.sampled + ' 条' : '来自刚导入的一段历史 ' + comp.sampled + ' 条'}),完成自我介绍:\n\n` + sourceText,
          maxTokens: 1600,
          effort: 'low',
        });
