...
export function selectL1(store, opts) {
  ...
  const scored = all
    .map((row) => {
      ...
      return { row, score: catW * heat + Math.min(KW_BONUS_CAP, kw * KW_BONUS_PER_HIT) + (row.importance >= 1 ? 0.8 : 0) + rich, heat, kw, rich };
    })
    .filter((x) => x.score > 0)
  ...
  for (const c of chosen) {
    ...
    items.push({ ...c.row, score: +c.score.toFixed(3), kw: c.kw, rich: c.rich, line });
