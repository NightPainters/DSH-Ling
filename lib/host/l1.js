...
export function selectL1(store, opts) {
  ...
  //   记忆行不冗余存 branch,靠 `session_meta.branch_id` 推导(见内部记忆树设计笔记 §2.2)。
  ...
  const scored = all
    ...
    .filter((x) => x.score > 0
      && !(x.row.source === 'dsh' && archived.has(String(x.row.conv_id)))
      && !forgotten.has(String(x.row.source) + '|' + String(x.row.conv_id)))
