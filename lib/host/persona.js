...
import { key as dedupeKey } from './rules.js'; // A4(1.5.2):出口复用写入侧同一把去重尺,不另发明归一化

/** A3(1.5.2):条目正文里的半角方括号转全角。
 *  段边界现在只剩"行首是不是 ["一条判据,正文混入半角 [ 会让模型误判成新段(也可借换行伪造段头)。
 *  选全角而非反斜杠:反斜杠在 markdown/模板层可能被再吃一层,而全角替换无歧义、观感不变。 */
function escSegBrackets(text) {
  return String(text).replace(/\[/g, '［').replace(/\]/g, '］');
}

/** A4(1.5.2):渲染出口去重 —— 保留首次出现、顺序不变,只作用于渲染,绝不回写存储。
 *  归一化复用 rules.js 的 key()(写入侧同一把尺),避免两套归一化各自漂移。
 *  空键(正文纯标点)一律保留:宁可不去重,也不在渲染期静默吞掉一条真条目。 */
...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  assistant: {          // S7(2026-09-18 与用户定):小助手(本机小模型)地址与模型 —— 三级覆盖的第 1 级
    baseUrl: '',        // 空 = 落到环境变量 DSH_LING_ASSISTANT,再落到内置默认(loopback:11434)
    model: '',          // 空 = 落到环境变量 DSH_LING_ASSISTANT_MODEL,再落到 qwen3.5:9b
  },
...
export function assemblePersona(settings, modeKey) {
  ...
  // A1(1.5.2):按「段」收集 —— 段与段之间空一行(join('\n\n')),段内条目之间仍单换行。
  // 为什么不把每条 push 都用 '\n\n' 连接:那样条目之间会变成双倍空行(段内反而更糊)。
  const segs = [];
  const self = [];
  ...
  segs.push([`[身份·${coreNameOf(p.aiName)}]`, ...self].join('\n'));
  if (Array.isArray(p.bottomLines) && p.bottomLines.length) {
    const seg = ['[底线]'];
    for (const r of p.bottomLines) if (typeof r === 'string' && r.trim()) seg.push(`- ${escSegBrackets(r.trim())}`);
    segs.push(seg.join('\n'));
  }
  ...
  if (rules.length) {
    // A2(1.5.1):段头去掉元描述 —— 括号里是**写给主人看的说明**,不该进模型上下文;
    // 且「可直接追加」读起来像一条授权指令,模型可能误读成"我可以自己加规矩"。
    // 契约变更:注入面段头自此固定为纯标签 [规矩] / [习惯](tests/rules.test.mjs 同步)。
    segs.push(['[规矩]', ...dedupeByKey(rules).map((r) => `- ${escSegBrackets(r)}`)].join('\n'));
  }
  if (habits.length) {
    segs.push(['[习惯]', ...dedupeByKey(habits).map((h) => `- ${escSegBrackets(h)}`)].join('\n'));
  }
  ...
  if (awaiting.length) {
    segs.push(`[待我回应] 用户提的习惯候选:${awaiting.map((h) => `「${h.text.trim()}」`).join('、')}`
  ...
  if (typeof style === 'string' && style.trim()) {
    ...
    segs.push(`[${mode === 'work' ? '工作' : '生活'}模式]${style.trim()}`);
  }
  return segs.join('\n\n');
}
