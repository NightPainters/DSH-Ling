...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  memory: {
    ...
    l1BudgetTokens: 1200,
    l1MaxItems: 8,
    l1MaxLineChars: 180,   // 单行上限:防止一条超长摘要吃满整个预算(截断优先落在句末)
  ...
  habits: {             // 习惯生成器(通道 A 数出来 / 通道 B 想出来)
    scanMinHits: 3,           // A:同类纠正至少几次
    scanMinSessions: 2,       // A:至少跨几个会话(单会话里被说三遍不算模式)
...
export function assemblePersona(settings, modeKey) {
  ...
  if (awaiting.length) {
    out.push(`[待我回应] 尝生提的习惯候选:${awaiting.map((h) => `「${h.text.trim()}」`).join('、')}`
