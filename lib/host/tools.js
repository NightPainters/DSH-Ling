...
import {
  opTreeRead, opBranchMembers, opBranchCreate, opBranchRename, opBranchReparent,
...
/** 短 id(br:b46c191e-… → b46c191e)。B4(实测报告):tree 概览此前只印名字,
 *  而 branch_edit 只吃 id ⇒「看一眼树 → 改一条枝」中间必须另找办法拿 id。 */
function shortId(id) {
  const s = String(id || '');
  const m = s.match(/^[a-z]+:(.{8})/);
...
export function checkBranchEdit(args) {
  ...
  return {
    ok: true, action, id, to, name, reason,
    kind,
    parentId: String(args?.parentId ?? '').trim(),
...
function renderTreeRead(v) {
  if (!v.ok) {
    return ({
      ...
      'no-match': `没有 id 为「${v.id}」的枝 —— 是不是把枝名当 id 传了?先用 action=tree 拿真 id(概览行尾那 8 位)。`,
  ...
  return `记忆树概览(共 ${v.branches} 条枝;▣ 主干 ◆ 主脉 · 枝;行尾是短 id):\n${(v.lines || []).join('\n')}`
    + linkBlock
...
function renderBranchEdit(v) {
  if (!v.ok) {
    const why = {
      ...
      'no-match': '这条枝(或这条边)不存在,没有改动任何东西。',
      cycle: '不能把一条枝挂到它自己的子孙下面。',
      'is-trunk': '主干不能删、也不能改名。',
    }[v.reason];
    return (why || `未执行(${v.reason || '未知'})。`) + '\n(库没有变化)';
  }
  const what = {
    create: `建了一条${v.kind === 'vein' ? '主脉' : '枝'}「${v.name}」`,
    rename: `把「${v.from || v.id}」改名为「${v.name}」`,
    reparent: `把「${v.id}」挂到了「${v.parentId}」下(内容零变化)`,
    link: `连了一条边:${v.from || v.id} ↔ ${v.to}`,
    unlink: `断开了边:${v.from || v.id} ↔ ${v.to}`,
    weight: `把「${v.id}」的枝系数调到了 ${v.weightScale}`,
    delete: `删掉了枝「${v.name || v.id}」`,
  }[v.action] || '改完了';
...
export const BRANCH_EDIT_SPEC = {
  ...
  description:
    ...
    + 'reparent(并脉:把某条枝挂到主脉下,需 id+parentId,内容零变化)/ link(连边,需 id+to,'
    ...
    + 'delete(删枝,需 id;非空枝要 force)。所有改动都记在「器灵」名下,'
    + '并出现在他下一次开口前的注入里。',
  parameters: {
    type: 'object',
    properties: {
      ...
      id: { type: 'string', description: '要改的枝 id(tree_read 的概览行尾就是短 id)' },
      ...
      parentId: { type: 'string', description: 'create / reparent 时的父枝 id(默认主干)' },
...
function renderForget(v) {
  if (!v.ok) {
    const why = {
      ...
      'no-reason': `遗忘必须给 reason(至少 ${MIN_REASON} 个字,写进归档日志)。`,
...
export const MEMORY_FORGET_SPEC = {
  ...
  description:
    ...
    + 'action:forget(默认;需 source+convId+reason)/ undo(取消遗忘;需 source+convId)/ list(看已遗忘清单)。'
  ...
  parameters: {
    ...
    properties: {
      ...
      source: { type: 'string', description: '来源域:dsh | dsweb | import' },
...
export const MEMORY_BACKFILL_SPEC = {
  ...
  parameters: {
    ...
    properties: {
      ...
      source: { type: 'string', description: '(可选)灌回到哪个来源域;缺省用归档里记的' },
    ...
    execute: guard(async (args) => {
      ...
      return {
        ...
        links: (links || []).slice(0, 20).map((l) => ({
          from: shortId(l.from), to: shortId(l.to), kind: l.kind, note: l.note,
        })),
      };
    }),
  };
    ...
    execute: guard(async (args) => {
      ...
      if (payload.ok) {
        ...
        return {
          ...
          name: (isCreate || c.action === 'rename') ? c.name : undefined,
          parentId: (isCreate || c.action === 'reparent') ? (c.parentId || TRUNK_ID) : undefined,
          kind: (isLink || isCreate) ? c.kind : undefined,
          weightScale: c.action === 'weight' ? c.weightScale : undefined,
        };
      }
      return payload;
    }),
