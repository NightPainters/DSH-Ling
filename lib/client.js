        ...
        function nodeRow(n, depth) {
          ...
          row.onclick = function (ev) {
            if (ev && (ev.ctrlKey || ev.metaKey)) {
              ...
              // 无 Ctrl:主轴换到本行(再点一次本行 = 取消选中);旧主轴降级进额外集合,视觉上不掉色
              if (isPrimary) { st.treeSel = ''; }
              else {
                if (st.treeSel) {
                  var oi = st.extraSel.indexOf(st.treeSel);
                  if (oi < 0) st.extraSel.push(st.treeSel);
                }
                var si = st.extraSel.indexOf(n.id);
                if (si >= 0) st.extraSel.splice(si, 1);
                st.treeSel = n.id;
              }
            }
            paintTree();
          ...
          row.title = '记忆中心始终只读;点击选中,Ctrl + 单击 = 加选/减选。想改归属请到右侧复盘栏「🔓 解锁编辑」后拖动。';
          box.appendChild(row);
