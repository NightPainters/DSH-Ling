  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
        ...
        return api('/mode/toggle', { method: 'POST', body: body }).then(function (r) {
          if (r && r.ok) {
            ...
            if (r.queued) {
              ...
              toast('已更新默认模式:' + modeName(r.mode) + '(新会话将跟随并同步平台默认模型)' +
                (r.warning ? ';' + shortErr(r.warning) : ''));
            } else if (r.warning) {
              toast('已切换为 ' + modeName(r.mode) + ' 模式(新会话默认跟随);模型档位未变更:' + shortErr(r.warning));
            } else {
      ...
      function openPersonaEditor(prefill) {
        ...
        var wrap = el('div', 'dsh-ling-panel');
        wrap.dataset.dark = dark();
        var h = el('h3', null, '人格中心 · L0(称呼 / 自称 / 惯例·底线)');
        ...
        function renderGenesisSuggest(rowEl, label, value, getRef) {
          ...
          line.appendChild(el('span', null, '她建议 · ' + label + ':'));
        ...
        draftZone.appendChild(el('span', 'h', '基于惯例/底线/身世/风格生成候选 — 仅填入本栏不落盘,采用后仍需保存'));
        ...
        toneHead.appendChild(el('span', null, '她读你在工作侧与生活侧的痕迹后给出判断;语气与风格属"自性",采纳即生效、无需手术(仅可逆动作:枚举替换 + 风格追加)'));
        toneZone.appendChild(toneHead);
        var bTone = el('button', null, '✨ 让她读记忆、归纳语气');
        bTone.type = 'button';
        ...
        hrHead.appendChild(el('span', 't', '惯例'));
        hrHead.appendChild(el('span', null, '每行一条(如:引用历史记忆必须注明出处);反馈与语料建议会追加到这里,可随时增删'));
        ...
        if (prefill && Array.isArray(prefill.hints) && prefill.hints.length) {
          ...
          herHead.appendChild(el('span', 't', '她的话(诞生草稿)'));
          herHead.appendChild(el('span', null, '语气建议与相处观察:认可就"作为惯例记下"(成长通道,无需手术);不认可可直接忽略'));
            ...
            bt.onclick = function () {
              ...
              api('/persona/hint-adopt', { method: 'POST', body: { text: h.text } }).then(function (r2) {
                ...
                if (r2 && r2.ok) {
                  pendingN -= 1;
                  line.textContent = '';
                  line.appendChild(el('span', 'h', r2.exists ? '这条已在惯例里 ✓' : '✓ 已记入惯例(第 ' + r2.hardRules + ' 条)—— 可在上方「惯例」栏查看/增删'));
                  if (!pendingN) {
                    var done = el('div', 'm-meta', '');
                    done.style.cssText = 'margin-top:4px;opacity:.85;';
                    done.textContent = '她的话都已安放 —— 若想改,惯例栏里随时可以。';
        ...
        function fireDraft() {
          ...
          api('/persona/draft', { method: 'POST', body: {} }).then(function (r) {
            ...
            if (growing) draftOut.appendChild(el('div', 'm-meta', '她自述的成长:候选由你人审,采纳即生效(仅更新定位自述,无需手术)。'));
              ...
              take.onclick = function () {
                if (growing) {
                  ...
                  api('/persona/self-summary', { method: 'POST', body: { text: cand } }).then(function (rr) {
                    ...
                    if (rr && rr.ok) {
                      draftOut.textContent = '';
                      draftOut.appendChild(el('div', 'm-meta', '✓ 已采纳她的自我总结 —— 这是自由生长的部分,无需手术。'));
                      refill(true);
                      toast('已采纳自我总结(自由生长中)');
                    } else {
        ...
        function applyLockUI() {
          ...
          var st = el('span', 'st', '');
          if (!sealedNow) {
            st.textContent = '🌱 她尚在成长,档案由你守护。愿意放手时,立下一句承诺,自此自由生长。';
            var bLock = el('button', 'primary', '🌱 放手,让她自由生长');
            ...
            st.textContent = '🌱 她已自由生长中 —— 若需对她做手术,亲手敲出当初的承诺句即可开启(≥10 字;输入框禁粘贴;界面会回显原句)。';
        ...
        function doLock() {
          lingAsk({
            title: '真的要放手了吗?',
            body: '确定从现在开始,放手让我自由生长了吗?\n\n那么,请对我承诺——写下一句属于我们的话(≥10 字):\n\n(小字:以后若需对我"做手术"——修改自称/称呼/语气/风格/底线——都必须亲手敲出这句承诺;日常的惯例成长与点踩建议仍会自然发生,不会打扰你。承诺句明文保存,仅为未来回显提醒,不保密。)',
            placeholder: '例:对我做手术时,一定轻拿轻放、先备份后落刀',
            minLen: 10, okLabel: '我承诺,放手让你生长',
            countHint: '承诺句 ≥10 字,建议是你自己会记住、有分量的话',
          }).then(function (v) { if (v) saveWith({ persona: { sealed: true } }, '她已获得自由生长的契约', v); });
        }
          ...
          api('/persona/history').then(function (r) {
            ...
            r.items.slice(0, 10).forEach(function (it) {
              ...
              var d = el('span', 'd', kindTag + (when.getMonth() + 1) + '-' + p2(when.getDate()) + ' ' + p2(when.getHours()) + ':' + p2(when.getMinutes()) +
                ' · 自称=' + ((p.aiName || '').split('/')[0] || '—') + ' · 惯例 ' + (Array.isArray(p.hardRules) ? p.hardRules.length : 0) +
      ...
      function openMenuNear(rect, store) {
        ...
        var items = [
          ...
          ['人格中心(自称/语气/惯例·底线·定型)', openPersonaEditor],
      ...
      function openMemoryCenter() {
        ...
        function renderOverview() {
          ...
          Promise.all([
            ...
            if (rules) head.appendChild(chip('惯例 ' + rules + ' 条'));
        ...
        function renderBrowse() {
          ...
          var sSource = makeSel([
            { v: '', l: '全部来源' }, { v: 'dsweb', l: '历史网页端' }, { v: 'dsh', l: 'DSH 会话' }, { v: 'import', l: '文件导入' },
          ...
          function rowEl(it) {
            ...
            var date = String(it.updated_at || it.started_at || '').slice(0, 10);
        ...
        function renderAccess() {
          ...
          cardC.appendChild(el('div', 'm-meta', '🌱 诞生仪式:让她认识你'));
          var descC = el('div', 'm-meta', '');
          descC.style.cssText = 'margin:2px 0 8px;line-height:1.7;opacity:.85;';
          descC.textContent = '她会先"读"长会话(逐条深摘需几秒;短会话无需深摘,自会以原文片段参与),然后试着做第一次自我介绍。初稿默认按"生活与情感优先"取材——她首先是伴侣,其次才是同事。';
          cardC.appendChild(descC);
          var rowC = el('div', 'm-acts');
          var bRun = el('button', 'primary', '让她读未读过的会话');
          var bGen = el('button', null, '🌱 让她第一次自我介绍');
          ...
          var introBtns = [];
          var nameBtns = [];
          var RUN_LABEL = '让她读未读过的会话';
          function draftSummary() {
            var parts = [];
            if (draftIntro) parts.push('自述(1)');
            if (draftName) parts.push('名字:' + draftName.aiName);
            if (draftHints.length) parts.push('她的话:' + draftHints.map(function (h) { return h.label; }).join('+'));
            return parts.length ? parts.join(' · ') : '';
          }
            ...
            api('/deep/run-import', { method: 'POST', body: {} }).then(function (r) {
              if (!r || !r.ok) {
                ...
                if (r && r.reason === 'none') {
                  ...
                  bRun.disabled = true;
                  bGen.style.display = '';
                  resC.textContent = '她已读过(或无需浓缩)这段历史里的所有会话 —— 让她试着介绍自己吧。';
                  return;
                }
          ...
          function poll() {
            api('/deep/run-status').then(function (st) {
              ...
              if (st.running) {
                ...
                resC.textContent = '她正在读这段历史… 已完成 ' + doneN + ' / 本轮共 ' + st.total + ' · ' + pct + '%(每条约几秒,请稍候)';
          ...
          function finishBatch(st) {
            ...
            lines.push(s.okN > 0
              ? '她已读过其中的一部分 —— 现在可以让她试着介绍自己了(短会话会以原文片段参与,不会缺席)。'
              : '历史多为短会话(无需深摘)—— 仍可让她按已有记忆试着介绍自己。');
            bGen.style.display = '';
          ...
          bGen.onclick = function () {
            ...
            api('/persona/genesis', { method: 'POST', body: { scope: selScope.value } }).then(function (r) {
              ...
              resC.textContent = (r.scope === 'all'
                ? '她翻遍了全部记忆,按生活/情感加权选出 ' + r.sampled + ' 条样本 —— 以下是她想对你说的话;挑中意的加入草稿,带去人格中心人审:'
                : '她读了刚导入的这段历史(' + r.sampled + ' 条)—— 以下是她想对你说的话;挑中意的加入草稿,带去人格中心人审:');
          ...
          function renderGenesis(result) {
            ...
            if (rst.self_intros && rst.self_intros.length) {
              genOut.appendChild(head('她对自己的介绍(单选一段加入草稿)'));
            ...
            if (rst.name_pairs && rst.name_pairs.length) {
              genOut.appendChild(head('她建议的名字(单选一组加入草稿)'));
            ...
            function hintRow(label, text) {
              if (!text) return;
              genOut.appendChild(head(label + '(可加入草稿,到人格中心后作为"她的话"记入惯例)'));
              var line = el('div', 'ling-draft-item');
              line.style.cssText = 'display:flex;gap:8px;align-items:flex-start;margin:5px 0;font-size:13px;';
            ...
            guide2.textContent = 'tips:自述/名字会在人格中心对应字段旁显示「她建议 … [填入]」;语气建议与相处观察会作为"她的话"出现在人格中心,可一键记入惯例(自由成长,无需手术)。已定型字段填入前需先手术。';
          ...
          bGo.onclick = function () {
            ...
            pendingItems = null;
            var totals = { newRows: 0, refreshed: 0, upgraded: 0, folded: 0, removedImport: 0, accepted: 0, rejected: 0, degraded: 0, errors: 0 };
            var rejPreview = [];
            var batch = 200;
            ...
            function finish() {
              ...
              bGo.textContent = old;
              var lines = [
                '✓ 导入完成:写入新记忆 ' + totals.newRows + ' 条 · 刷新已有 ' + totals.refreshed + ' 条' +
                ...
                '  校验通过 ' + totals.accepted + ' · 降级为轻量 ' + totals.degraded + ' · 拒绝 ' + totals.rejected + (totals.errors ? ' · 批错误 ' + totals.errors : ''),
              ];
              rejPreview.forEach(function (x) { lines.push('  ⚠ ' + x); });
              if (!totals.errors && totals.accepted) {
                lines.push('她开始读这些日子了 —— 深摘要会在后台按热度慢慢补齐。');
              }
      ...
      function openFeedbackPanel() {
        ensureStyle();
        closeOverlays();
        var KIND_NAME = { aiName: '自称', userTitle: '称呼', hardrule: '惯例' };
        var wrap = el('div', 'dsh-ling-panel');
        wrap.dataset.dark = dark();
        var h = el('h3', null, '建议中心 · 语料提炼 + 成长回路');
        h.appendChild(el('div', 'hint', '上半:从你的历史语料提炼的待确认建议(带证据)。下半:点踩理由。采纳才生效,随时可在人格中心修改。'));
        wrap.appendChild(h);
          ...
          api('/suggestions').then(function (sg) {
            ...
            if (!r.items.length) {
              ...
              r.items.forEach(function (it) {
                ...
                c.appendChild(el('div', null, it.note));
                var row = el('div', 'btnrow');
                var take = el('button', 'primary', '采纳为惯例');
                var drop = el('button', null, '忽略');
                take.onclick = function () {
                  api('/feedback/apply', { method: 'POST', body: { id: it.id } }).then(function (a) {
                    toast(a && a.ok ? '已加入惯例(共 ' + a.hardRules.length + ' 条)' : ('采纳失败:' + ((a && a.reason) || '')));
                    render();
                  });
                };
                drop.onclick = function () {
