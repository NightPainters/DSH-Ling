...
(function () {
  ...
  // ⚠️ 下面这个 id 必须【逐字等于本包的 npm 包名】。客户端模块表以「包名」为 row id
  // (dsh-client-modules/lib/index.js:884 graphRow(packageName,…) → :649-652 取 record.entry.id),
  // 而注册 id 是脚本自己报的;两者不符时,客户端判定"该 row 的模块没到货",会回退去取
  // 单资源 URL 把同一份字节**再执行一遍**(client.js:624-626、:639-640),第二次注册撞上
  // this.factories 去重守卫 ⇒ 抛 `client-modules: duplicate factory registration for "…"`
  // ⇒ `web boot: 1 entry did not activate` ⇒ **前端整片黑屏**(2026-09-26 改名事故实测两次)。
  // 对照:@linxin666/* 与 dsh-better-sidebar 的 bundle 都逐字写自己的完整包名。
  // ⇒ 包名一旦再改,这里必须同批改;tests/unit.mjs 有断言守着。
  window.__ModuleLoader__.load({
    id: '@nightpainters/dsh-ling',
    factory: function (require) {
      ...
      /** `ms` 可选:不传时沿用原样 3.2s(既有调用全部不传)。需要长一点的是**覆盖/归档**这类
       *  "数据已经变了"的回执 —— 3.2s 读不完一句带归档目录的话(审计 E10)。 */
      function toast(msg, ms) {
        ...
        // 后端回执里的 `message` 允许带 `**强调**` 标记(如「**覆盖了上一版主脉记忆**」),
        // 而 toast 是纯文本容器 ⇒ 显示层去掉标记,别把星号糊给主人。
        // (非字符串原样交给 el(),不改变任何既有调用的行为:全文件现有 toast 文案里没有 `**`。)
        t.appendChild(el('div', 'mi', (typeof msg === 'string') ? msg.replace(/\*\*/g, '') : msg));
        document.body.appendChild(t);
        setTimeout(function () { t.remove(); }, Number(ms) > 0 ? Number(ms) : 3200);
      }

      /** 只在**自有属性**上取表项(1.5.2 收尾,纯健壮性 —— 不改任何既有分支行为):
       *  `map[key]` 会走原型链,key 恰为 `toString` / `constructor` / `__proto__` 这类原型键时,
       *  拿回来的是 Object.prototype 上的成员(一个函数)⇒ 会被当成"已知原因"直接印进 toast。
       *  全文件**只此一处**实现:apiOk(写操作回执)与 doDistill(提炼 reason→提示)共用,勿再抄第二份。 */
      function ownGet(map, key) {
        return (map && key && Object.prototype.hasOwnProperty.call(map, key)) ? map[key] : '';
      }

      /** 写操作回执的**统一判读**(审计 E6/E13,2026-09-25 红蓝对抗)。
       *  为什么需要它:`api()` 只在 fetch **reject** 时抛错,后端"HTTP 200/400 + {ok:false, reason}"
       *  这条**业务失败**路径不会抛 —— 调用方若只看 `.then(` 就会把失败当成成功(谎报)。
       *  返回 { ok, err }:ok=false 时 err 是一句可直接进 toast 的中文;
       *  `extra` 用于给已知 reason 补**人话**(如 unlink 的 no-match 其实是"这条边已经没了")。
       *  ⚠️ 后端若已带中文 `message` 就优先用它,不要用 reason 覆盖掉更准的说明。 */
      function apiOk(r, extra) {
        if (r && r.ok) return { ok: true };
        var code = r && r.reason;
        var m = (r && r.message) ? String(r.message)
          : (code ? (ownGet(extra, code) || String(code)) : '未知(网关没给原因)');
        return { ok: false, err: m, code: code || '' };
      }
      ...
      function openMemoryCenter() {
        ...
        var st = {
          ...
          // 1.5.2 收尾:`treeScroll` 原先**只写不声明**(onScroll 里写、重绘时 `if (st.treeScroll)` 读)。
          // 读 `undefined` 是假值、被 `if` 挡住,故无 NaN / 无实害 —— 但"字段从未初始化"会让下一个
          // 读它的人以为是笔误(或以为是别处初始化过)。显式置 0:行为完全一致(0 与 undefined 同假)。
          treeScroll: 0,
          // 审计 E12(1.5.1 红蓝对抗,2026-09-25):**Ctrl 多选整层删除**。
          // C#15 把记忆中心改成**只读视图**、删掉拖放之后,`extraSel`(Ctrl 加选集合)就只剩
          // "把行染色"这一个用处 —— 全文件再无任何消费者(没有任何批量动作读它)。
          // 于是它从"双真值缺陷"变成了**更坏的东西:一个骗人的提示** —— row.title 仍在教主人
          // 「Ctrl + 单击 = 加选/减选」,而加选之后什么也做不了(树里唯一的动作是"选中一行,
          // 看它的横向连边",那是单选语义)。
          // 判定:选"**删提示 + 删实现**"而不是"让它生效" —— 这个视图没有批量动作可挂,
          // 后端也没有被它调用的批量端点;若硬把 Ctrl 多选做成"真选中"却无后续动作,一样是骗人。
          // 故现在:treeSel 是**唯一**选中真值,点击 = 单选(再点本行 = 取消)。
          // 1.5.2 收尾:上一段注释里那行 `treeData: null, …, reviewLog: null,` 曾**逐字重复**
          // 出现一次(插注释时的编辑事故)。重复键后者胜、两份内容又完全相同 ⇒ 无行为差异,
          // 但会让下一个人以为两处字段不同、改错地方。已删掉多余的那份,只留上面一处。
          // 注意:`reviewLog` 字段本身**保留** —— 虽然全文件目前零读者,但它是状态字段(见报告)。
        ...
        // E1(1.5.1 红蓝对抗):这里原有一个 Ctrl+Z 监听,调 undoLastDrop() 撤回"记忆中心的上一次拖动"。
        // 但记忆中心自 1.4.0 起**只读**(拖放落点 doDrop 已删),`st.lastDrop` 再无写入点 ⇒
        // 那个函数恒返回 false、早返,监听**从来没起过作用**(一直是浏览器原生撤销在兜底)。
        // 连同函数一起删除:记忆中心不产生可撤销的操作,撤回语义**归侧栏复盘栏**(见 undoRef)。
        wrap.tabIndex = 0;
        ...
        function renderOverview() {
          ...
          var counts = {};
          var meta = {};
          // 审计 E7(2026-09-25 红蓝对抗):概览页原来是**永久停在「加载器灵概览…」**的坑 ——
          // qOne 六路各自有 .catch,但 /state、/suggestions、/feedback、/deep/status 这**四路是裸调用**,
          // 任一 reject(网关不可达 / 宿主未重启)都会让整个 Promise.all 直接 reject,渲染 .then 永不执行。
          // 修法:四路各补 `.catch`(留 warn + 记进 ovFailed),让**能拿到的部分照常渲染**;
          // 拿不到的统计不再假装是 0,而是在面板底部用一行 m-meta 小字如实说明缺哪几项。
          // (呈现选择:面板内**行内小字**,不用顶部 toast —— 这是"页面状态"不是"一次操作的回执";
          //  与同文件建议中心/规矩区已有的 '网关不可达'/'加载失败...' 行内说明同一套语言。)
          var ovFailed = [];
          function ov(pathLabel) {
            return function () { if (ovFailed.indexOf(pathLabel) < 0) ovFailed.push(pathLabel); };
          }
          ...
          Promise.all([
            ...
            api('/state?scope=global&l0Preview=1').then(function (r) { meta.state = r; }).catch(ov('人格状态')),
            api('/suggestions').then(function (r) { meta.sugg = r; }).catch(ov('语料建议')),
            api('/feedback').then(function (r) { meta.fb = r; }).catch(ov('点踩反馈')),
            api('/deep/status').then(function (r) { meta.deep = r; }).catch(ov('深摘状态')),
          ]).then(function () {
            body.textContent = '';
            var st8 = meta.state || {};
            var per = st8.persona || {};
            ...
            g.appendChild(line);
            // 审计 E7:上面四路若没拿到,不能假装数字是 0 —— 行内如实说缺哪几项。
            if (ovFailed.length) {
              var missRow = el('div', 'm-meta', '⚠ 以下数据本次没读到(' + ovFailed.join(' / ')
                + '),牌子上的 0 或「?」不代表真实值 —— 网关不可达时点「刷新」或在宿主重启后再看。');
              missRow.style.cssText = 'margin-top:6px;line-height:1.6;opacity:.85;color:#d9a441;';
              g.appendChild(missRow);
            }
        ...
        function nodeRow(n, depth) {
          ...
          // ---- 选中:单击选中(唯一真值 treeSel);再点本行 = 取消选中 ----
          // 审计 E12:Ctrl 加选/减选那一套连同 `st.extraSel` 一起删除(理由见 st 初始化处的长注释):
          // 这个视图只读、没有批量动作,多选既无消费者又在骗人。行内高亮只由 treeSel 决定。
          var isPrimary = (st.treeSel === n.id);
          if (isPrimary) {
            row.style.cssText += 'background:rgba(197,139,47,.20);outline:1px solid rgba(197,139,47,.55);';
          }
          row.onclick = function () {
            // 单选:直接点击 = 选中本行;再点本行 = 取消选中。
            // (Ctrl/⌘ + 单击不再有特殊含义 —— 2026-09-25 与提示文案同步删除,避免"按了没反应")
          ...
          row.title = '记忆中心始终只读;点击一行 = 选中它(再点一次取消选中),选中后下方会展开它的横向连边。想改归属请到右侧复盘栏「🔓 解锁编辑」后拖动。';
          box.appendChild(row);
        ...
        /**
         * E1(1.5.1 红蓝对抗):**这个函数已删除**。
         * 它读 `st.lastDrop`(注释称"由侧栏复盘栏写入"),但**全文件没有任何写入点** ——
         * 1.4.1 清 `doDrop` 死代码时把赋值一起删了、却留下了读取侧,于是 `if (!u) return false`
         * 恒真早返:记忆中心的 Ctrl+Z 从来没有生效过(连 `preventDefault` 都不会执行,
         * 一直是浏览器原生撤销在兜底)。既然记忆中心已按 1.4.0 的裁决**只读**,
         * "撤回记忆中心的一次拖动"就没有语义了 —— 撤回归侧栏(见 undoRef)。
         */
        ...
        function buildReviewPrompt() {
          ...
          // 交叉复核补丁(2026-09-26,复核第 3 项):上一行如实写「N 条」,这里只列 12 条且无声。
          // 这段是**交给器灵的开场材料** —— 它少看到的那几条,会直接影响它对"该并哪两条"的判断,
          // 所以截断可以留(整篇 prompt 不宜无限长),但必须自报截了多少。
          var PEND_LIST_MAX = 12;
          if (pend.length > PEND_LIST_MAX) {
            out.push('（下面只列前 ' + PEND_LIST_MAX + ' 条,还有 ' + (pend.length - PEND_LIST_MAX) + ' 条未列出）');
          }
          pend.slice(0, PEND_LIST_MAX).forEach(function (c) {
        ...
        function paintTree() {
          ...
          if (selLinks.length) {
            ...
            selLinks.forEach(function (l) {
              ...
              bUn.onclick = function () {
                // 审计 E6(2026-09-25 红蓝对抗):此前**不看 r.ok、也无 .catch** ——
                // 后端在边不存在时明确回 400 `{ok:false, reason:'no-match'}`(见 tree-ops `opVeinUnlink`),
                // 旧写法照样 toast「已断开」并重载 ⇒ 主人以为断了、其实一条没断(假回执)。
                // 呈现选择:与同面板 delSnapshot 的既有风格一致 —— 顶部 toast 前缀「✗」+ 中文原因,不新增红条。
                var UNLINK_REASON = {
                  'no-match': '这条连边已经不存在了(可能刚在别处断过)',
                  'bad-id': '枝 id 不对 —— 刷新后重试',
                  'db': '记忆库写入错误',
                };
                api('/veins/unlink', { method: 'POST', body: { from: linkFrom(l), to: linkTo(l) } })
                  .then(function (r) {
                    var j = apiOk(r, UNLINK_REASON);
                    if (!j.ok) { toast('✗ 断开失败:' + j.err); return; }
                    toast('已断开');
                    st.treeData = null;
                    loadTreeAll();
                  })
                  .catch(function (e) {
                    console.warn('[dsh-ling] 断边失败', l, e);
                    toast('✗ 断开失败:' + ((e && e.message) || '网关不可达'));
                  });
              };
              row.appendChild(bUn);
          ...
          // 交叉复核补丁(2026-09-26,复核第 3 项 —— 与 E3 **同一个根因**,只是阈值从 6 换成了 30):
          // 上面 cHead 如实写「待复盘 N / 全部 M」,这里却硬截 30 条,且**没有任何"还有 N 条"的提示**
          // ⇒ 第 31 条之后的矛盾在界面上不可见(既看不到裁定状态,也无从处理)。
          // 与同文件 `SIDE_TREE_MAX`(C#14)、下方 `PEND_SHOWN` 同款修法:保留截断(免得一次铺几百张卡),
          // 但**把截掉的数量说出来** —— 标题与卡片数于是不再互相矛盾。
          var CONF_SHOWN = 30;
          if (conf.length > CONF_SHOWN) {
            var cMore = el('div', 'm-meta', '…共 ' + conf.length + ' 条,这里只画前 ' + CONF_SHOWN
              + ' 条(还有 ' + (conf.length - CONF_SHOWN) + ' 条未显示)');
            cMore.style.cssText = 'opacity:.6;margin-bottom:6px;';
            body.appendChild(cMore);
          }
          conf.slice(0, CONF_SHOWN).forEach(function (c) {
        ...
        function renderAccess() {
          ...
          bScan.onclick = function () {
            ...
            api('/dsh/backfill', { method: 'POST', body: {} }).then(function (r) {
              ...
              bScan.textContent = old;
              if (r && r.ok) {
                // BUG-2:0 候选 ≠ 这台机器没历史。宿主在零候选时会带 reason/note 回来,这里必须显式说出来,
                // 否则「文件名代次变了导致全漏」与「确实没存量」在界面上长得一模一样(静默两周的根因)。
                var noneScanned = !r.scanned;
                resA.style.cssText = 'width:100%;margin-top:6px;white-space:pre-wrap;';
                resA.textContent = (noneScanned ? '⚠ ' : '✓ ') + '候选 ' + r.scanned + ' · 顶层 ' + r.topLevel + ' · 新增 ' + r.created +
                  ' · 已存在跳过 ' + r.skippedExists + ' · 弱会话跳过 ' + r.skippedWeak +
                  (r.failed ? ' · 解码失败 ' + r.failed : '') + ' · 库内 DSH 共 ' + r.totalDsh + ' 条' +
                  (noneScanned ? '\n' + (r.note || '这台机器没有可导入的 DSH 历史会话') : '');
                toast(noneScanned ? ('没扫到 DSH 会话文件:' + (r.note || '本机没有历史')) : ('DSH 历史扫描完成:新增 ' + r.created + ' 条记忆'));
              } else {
          ...
          api('/suggestions').then(function (sg) {
            ...
            sg.items.forEach(function (it) {
              ...
              drop.onclick = function () {
                // 审计 E13(2026-09-25 红蓝对抗):此前 `api(...).then(function () { render(); })` ——
                // 不看 r.ok、无 .catch。后端 dismissCorpusSuggestion 为空 id 时回 `{ok:false, reason:...}`,
                // 旧写法会静默无反应(卡片还在,主人不知道为什么点不动)。
                // 呈现选择:与同面板「采纳」按钮完全一致的双路(成功/失败都 toast,失败带中文原因),
                // 让"忽略"和"采纳"两个按钮的反馈风格不再一轻一重。
                api('/suggestions/dismiss', { method: 'POST', body: { id: it.id } })
                  .then(function (r) {
                    var j = apiOk(r, { 'not-found': '这条建议已经不在了(可能刚被处理过)' });
                    if (!j.ok) { toast('✗ 忽略失败:' + j.err); render(); return; }
                    render();
                  }, function (e) {
                    console.warn('[dsh-ling] 忽略语料建议失败', it.id, e);
                    toast('✗ 忽略失败:' + ((e && e.message) || '网关不可达'));
                  });
              };
            ...
            if (!r.items.length) {
              ...
              r.items.forEach(function (it) {
                ...
                drop.onclick = function () {
                  // 审计 E13:同「忽略语料建议」—— 此前不看 r.ok、无 .catch ⇒ 点「忽略」失败静默。
                  api('/feedback/dismiss', { method: 'POST', body: { id: it.id } })
                    .then(function (r) {
                      var j = apiOk(r, { 'not-found': '这条反馈已经不在了(可能刚被处理过)' });
                      if (!j.ok) { toast('✗ 忽略失败:' + j.err); render(); return; }
                      render();
                    }, function (e) {
                      console.warn('[dsh-ling] 忽略点踩反馈失败', it.id, e);
                      toast('✗ 忽略失败:' + ((e && e.message) || '网关不可达'));
                    });
                };
                row.appendChild(take);
      ...
      function reviewPromptOf(td, conf) {
        ...
        out.push('');
        out.push('【待处理矛盾】' + (pend.length ? pend.length + ' 条' : '暂无'));
        // 交叉复核补丁(2026-09-26,复核第 3 项):与 `buildReviewPrompt`(记忆中心版)同一处缺陷 ——
        // 标题写 N 条、正文只列 12 条、无声。这两份 prompt 是**并存的两套实现**(复核第 5 项),
        // 已各自漂移(这里少了「横向连边」与 detectedBy);本次只统一补上同一句自报截断的披露,
        // 不做合并 —— 合并属结构性改动,风险独立,留给主人裁决。
        var PEND_LIST_MAX = 12;
        if (pend.length > PEND_LIST_MAX) {
          out.push('（下面只列前 ' + PEND_LIST_MAX + ' 条,还有 ' + (pend.length - PEND_LIST_MAX) + ' 条未列出）');
        }
...
var lingCenterReload = null;   // 侧栏改完树后通知记忆中心重载(两侧同源,2026-09-22)
      ...
      function LingReviewBody() {
        ...
        // 审计 E10(1.5.1 红蓝对抗):「这条主脉**已经**有主脉记忆了」是**采纳前**就该知道的事。
        // 后端对同主脉的第二次采纳做的是"先把旧版归档、再覆盖"(用户 2026-09-25 裁决),可界面
        // 此前对"将要覆盖"零提示 ⇒ 唯一的发现途径是事后去翻「最近改动」。
        // 判据不靠后端加字段:`conv_overview.source='vein'` 的 `conv_id` **就是主脉 id**
        //(memory.js:1821 写入时 `conv_id: veinId`),所以查一次 `?source=vein` 就知道
        // 哪些主脉已经有主脉记忆了。失败时置空 = 不显示预警(**不假装安全,也不拦人**:
        // 真正的知情渠道是采纳回执里后端那句「覆盖了上一版…」,那条不依赖本请求)。
        var s12 = React.useState({});
        var veinMem = s12[0];      // { <主脉id>: true } —— 已经有主脉记忆的那些主脉
        var setVeinMem = s12[1];
        React.useEffect(function () {
          var alive = true;
          Promise.all([
            api('/tree').catch(function () { return null; }),
            ...
            api('/vein/suggestions?status=new&limit=100').catch(function () { return null; }),
            // E10:主脉记忆清单(只读,limit 上限就是 500,见 memory.js:1951)
            api('/memories?source=vein&limit=500').catch(function () { return null; }),
          ]).then(function (rs) {
            if (!alive) return;
            ...
            var vm = {};
            if (rs[6] && rs[6].ok) {
              (rs[6].items || []).forEach(function (it) {
                if (it && it.conv_id != null && String(it.conv_id)) vm[String(it.conv_id)] = true;
              });
            }
            setVeinMem(vm);
            setData({
        ...
        // ⚠️ E1(1.5.1 红蓝对抗):「↶ 撤回」的记账**必须放在 ref 里**。
        //   此前是 `var undoStack = []`(声明在函数体深处)—— 而**函数组件每次渲染都会
        //   重新执行整个函数体**,于是 sideReload() 触发的那次重渲染把记账清零,
        //   「↶ 撤回」永远报「没有可撤回的操作」。红队用**能通过的负对照**钉死了归因:
        //     不触发重渲染、用同一帧的按钮立刻点 → 撤回成功,POST 回原 parentId ✓
        //     让 sideReload() 走完、再点当前帧的按钮   → toast「没有可撤回的操作」,0 次请求
        //   ⇒ **记账逻辑本身是对的,错在存放位置**。
        //   位置纪律同上面的 Shift+C:必须落在本函数所有 early return **之前**(见 B24 说明)。
        ...
        function sideDrop(ids, targetId, isLink, fromId) {
          ...
          undoRef.current = [{ prev: prev, link: !!isLink }]; // 撤回信息必须在落库前记
          ...
          Promise.all(tasks).then(function (rs) {
            ...
            undoRef.current = [{ prev: prev.filter(function (p, i) { return !!(rs[i] && rs[i].ok); }), link: !!isLink }];
            if (!undoRef.current[0].prev.length) undoRef.current = [];
        ...
        // E1 修正:记账已上移到 undoRef —— 这里**不再声明**局部数组(声明一次就会被渲染清空一次)。
        function sideUndo() {
          if (!undoRef.current.length) { toast('没有可撤回的操作'); return; }
          var u = undoRef.current.pop();
          undoRef.current = [];
        ...
        if (!selX) {
          ...
          kids.push(h('div', { key: 'rp' }, [
            ...
            // E8 同根因的另两处(见审核卡那段长注释):这两个 `<select>` 原来 key 是**常量** `'s'`,
            // 而 `defaultValue` 随选中的节点变 ⇒ 换一个节点时 React 复用同一个 DOM 节点、
            // **不重挂载** ⇒ 框里留着的可能是**上一个节点的值**,而「并到」读的正是这个 DOM 值
            // ⇒ 会把当前节点并到没打算去的枝下(静默的结构改动)。key 里带上身份(和它的父枝)后,
            // "选中变了 / 它的父枝变了"都等价于重挂载,DOM 值恒等于 props。
            h('select', { key: 's' + selX.id + ':' + (selX.parentId || 'trunk'), ref: refRep, defaultValue: selX.parentId || 'trunk', style: inCss },
              flat.map(function (y) {
          ...
          kids.push(h('div', { key: 'lk' }, [
            ...
            h('select', { key: 'sl' + selX.id, ref: refLk, defaultValue: '', style: inCss },
        ...
        var doDistill = function (veinId) {
          // 1.5.2 定案:这里**只读**忙标志、**不再**补置位(`lingBusySet`)—— 前序把它记成"服务端
          // 锁的另一半",但 `lingBusy` 是 scan / judge / gather 三操作**共用**的一把锁,而服务端
          // BATCH_RUN 按 kind 分锁 ⇒ 前端置位会把三个操作互相挡死,且它没有任何可见状态。
          // 正确性由服务端锁闭合(reason 见 lib/host/api.js 的 BATCH_RUN 长注释)。
            ...
            .then(function (r) {
              if (!r || !r.ok) {
                var HINTS = {
                  ...
                  'llm-failed': '模型调用失败',
                  // C-06 的另一端(1.5.1 红蓝对抗,2026-09-26 补):这一轮 `/vein/distill` 刚上
                  // **单实例锁**(host/api.js:973-979 回 `reason:'busy'`)与**整批 deadline**
                  // (host/api.js:1026/1043 超时回 `reason:'timeout'`)⇒ 表里多出这两个 reason。
                  // 漏了它们不是"少一句提示":表尾兜底 `|| '提炼失败'` 会把**还在跑、等它就好**
                  // 误报成**提炼坏了**,主人据此要么放弃、要么以为是缺陷。
                  'busy': '上一条提炼还在跑 —— 等它出结果再点(重复点不会更快)',
                  'timeout': '提炼超时(模型太慢),本次已中止;可直接重试,不会产生重复候选',
                };
                // 1.5.2 收尾(纯健壮性,不改任何既有分支行为):表改用**自有属性**取,不再走
                // 原型链 —— 旧写法 `{…}[r.reason]` 在 reason 为 `toString` / `constructor` /
                // `__proto__` 这类原型键时会命中 Object.prototype 上的成员,把**函数源码**当
                // 中文提示 toast 给主人(概率极低,但形态很难看且无从解释)。
                var reason = (r && r.reason) || '';
                var hint = ownGet(HINTS, reason) || ''; // 自有属性取值(见 ownGet):别退回 `HINTS[reason]`
                // 「还在跑」不是错误(与 timeout 那种"已中止"不同)⇒ busy 不加 ✗;其余失败仍带 ✗。
                toast((reason === 'busy' ? '' : '✗ ') + (hint || '提炼失败'));
                return;
              }
        ...
        var resolveSug = function (id, action, text) {
          ...
          api('/vein/suggestion/resolve', { method: 'POST', body: { id: id, action: action, text: text } })
            .then(function (r) {
              // 审计 E9 的前端半 + E10(1.5.1 红蓝对抗):换用统一判读器 `apiOk`。
              // E9 的后端半已把 empty-text / bad-id / archive-failed 这类内部码译进中文 `message`,
              // 而旧写法只读 `reason` ⇒ 主人看到的是英文码。"`message` 优先"正是 apiOk 的既定口径。
              var v = apiOk(r);
              if (!v.ok) { toast('✗ ' + v.err); return; }
              if (action !== 'accept') { toast('已驳回'); sideReload(); return; }
              // 审计 E10(1.5.1 红蓝对抗):采纳的**成功**回执此前是一句泛泛的「已采纳」,而后端
              // 其实已经把该说的都写进 `message` 了 ——「覆盖了上一版主脉记忆(旧版已归档 <目录>),
              // 不是新增一条」与「名字是你手改过的,按约定保持不动」。**逐字显示它**,不要用自己的
              // 套话盖掉:覆盖是"数据真被换掉了",这是主人唯一的即时知情渠道(其次才是「最近改动」)。
              var msg = (r && r.message) ? String(r.message) : '已采纳 —— 成为主脉自己的记忆(带派生来源)';
              // 审计 T-1 的第二半(2026-09-26):后端 A-06 的后半会**自己复核**"归属真的落库了吗",
              // 只有它没落库时才回 `assigned:false`(host/api.js:1148-1153)—— 那时记忆写进去了、
              // 却没挂到主脉下,实际按主干显示。这是"主人必须立刻处理的偏差",不能和一句普通的
              // 「已采纳…」长得一模一样 ⇒ 警示前缀 + 与覆盖类同长的 9s 窗口。
              // 只在**显式** `false` 时警示:老后端不回该字段 = undefined,维持原样式,不误报。
              var unassigned = !!(r && r.assigned === false);
              toast((unassigned ? '⚠ ' : '') + msg,
                (unassigned || (r && r.replaced)) ? 9000 : 4000);   // 覆盖/未归属类的回执更长,给足看清的时间
              sideReload();
            })
            .catch(function (e) { toast('✗ 操作失败:' + ((e && e.message) || '网关不可达')); });
        };
        ...
        if (veinSugs.length) {
          ...
          var taRefs = {};
          // 审计 E3(1.5.1 红蓝对抗):**这里原来是 `veinSugs.slice(0, 6)`** —— 标题如实写
          // 「待审核 20 条」,却只画 6 张卡,没有分页、没有"还有 N 条"的提示 ⇒ 第 7 条之后的
          // 候选**永不可达**(既看不到、也无从采纳/驳回,只能等下一次提炼把队列撑得更大)。
          // 根因确认为**前端硬截断**:同一次渲染里标题就用 `veinSugs.length`(20),
          // 说明 20 条**已经在手里**;后端 `/vein/suggestions` 的 `limit` 是照传的
          //(host/api.js:948 `Number(qArg(req.url,'limit') || 50)`),不截 6。
          // 修法:候选是**待人工处理的可操作项**,隐藏它们等于丢工作 —— 故**全量渲染**,
          // 不设上限(队列规模由提炼频率决定,量级不大;每张卡只是 textarea + 两个按钮)。
          // 同时把请求的 limit 从 20 提到 100,免得"手里只有 20 条"时标题的 20 也是假的。
          // 审计 E8(1.5.1 红蓝对抗):**键必须是条目的身份,不能是下标**。
          //   `key:'vs'+i` 配上不受控的 `defaultValue`,咬人的方式是:列表一缩短(采纳/驳回掉一条、
          //   或重拉后顺序变了),React 按**下标**复用同一张卡片的 DOM ⇒ 第 i 个 textarea 里
          //   **留着上一条候选的正文**,而卡片元信息(→ 主脉名)与按钮闭包已换成第 i 条
          //   ⇒ 点「采纳」= 把 A 的正文写进 B 的主脉,**静默写错内容、界面没有任何迹象**。
          //   改用 `sg.id`(vein_suggestions 主键;后端 `Number(r.id)`,memory.js:1756)后,
          //   "身份变了"必然换卡片 ⇒ 正文与身份永远同源。
          //   为什么不改成受控输入:hooks 不能在 forEach 里调用 ⇒ 得在组件顶部养一张 id→draft 的
          //   state 表,那会让**每次按键**都重渲染整个面板(含最多 400 行节点树),还要额外处理
          //   "采纳/驳回后清草稿";而收益(正文与身份同源)与下面这行 key 完全重合。
          veinSugs.forEach(function (sg) {
            kids.push(h('div', {
              key: 'vs' + sg.id,
              ...
              // 审计 E10:这条主脉**已经有主脉记忆**时,「采纳」= 覆盖它(后端先归档旧版再覆盖)。
              // 这是"动手前的知情",与采纳回执里那句「覆盖了上一版…」一前一后互补 —— 后者证明
              // 真的覆盖了,前者让主人有机会**不点**。判据来自 `?source=vein` 的真实数据(见 s12)。
              veinMem[String(sg.veinId)] ? h('div', {
                key: 'wm',
                style: {
                  marginBottom: '4px', padding: '3px 6px', borderRadius: '6px', fontSize: '11px',
                  border: '1px solid rgba(197,139,47,.45)', color: '#e8c07a', background: 'rgba(197,139,47,.10)',
                },
              }, '⚠ 这条主脉已经有主脉记忆了 —— 采纳会覆盖它(旧版会先归档,目录写在回执与「最近改动」里)') : null,
              h('textarea', {
                // 同上:不受控输入的 `defaultValue` **只在挂载时**生效,同一 id 的草稿若在服务端变了
                //(重新提炼覆盖同一行),只改 props 不会刷新 DOM ⇒ 把正文本身放进 key,让"内容变了"
                // 等价于"重挂载",DOM 值恒等于 props(key 只是 React 的内部标识,不进 DOM、不进提示词)。
                key: 'ta' + sg.id + ':' + (sg.text || ''), rows: 3, defaultValue: sg.text || '',
        ...
        // 交叉复核补丁(2026-09-26,复核第 3 项):E3 修掉的是 `veinSugs.slice(0, 6)`,可**同一个根因**
        // 在这份矛盾列表上原地留着 —— 标题照实写 `pend.length`、下面只画 6 条、**没有**"还有 N 条"的
        // 提示 ⇒ 第 7 对之后在界面上不可见。修法与同文件 `SIDE_TREE_MAX`(C#14)那处完全同款:
        // 截断可以,但**口径必须写出来**,不能让人以为"就这么多"。
        var PEND_SHOWN = 6;
        if (pend.length > PEND_SHOWN) kids.push(h('div', { key: 'cm', style: { opacity: 0.6 } },
          '…共 ' + pend.length + ' 对,这里只画前 ' + PEND_SHOWN + ' 对(还有 ' + (pend.length - PEND_SHOWN) + ' 对未显示)'));
        pend.slice(0, PEND_SHOWN).forEach(function (c, i) {
        ...
        kids.push(h('div', { key: 'ops', style: { display: 'flex', gap: '6px', flexWrap: 'wrap' } },
          ...
          h('button', {
            ...
            onClick: function () {
              if (lingBusyNow()) { toast('正在跑,稍等…'); return; }
              lingBusySet(LING_BUSY_JUDGE_MS); // E14:判定是多轮长跑,用更宽的单轮窗口 + 每轮续期
       ...
       *
       * 审计 E14(1.5.1 红蓝对抗,2026-09-25):**这个兜底的口径原来是错的** ——
       * 它被写成了"单次操作的总时长上限 3 分钟",可 `judge()` 的最坏形态是
       * **40 轮 × 每轮一个 HTTP 请求**(`MAX_ROUNDS=40`),而后端一个请求的整批上限
       * 就是 `BATCH_MS.judge = 120s`(host/api.js:92),再加在飞那一条的收尾 ⇒ 
       * 40 轮的理论总时长**远超 3 分钟**。于是一到 3 分钟锁就自动解除,主人再点一次
       * 「✦ 模型判定」就能**并发跑第二个判定循环**(后端有单实例锁,第二个循环会拿到
       * `{ok:false, reason:'busy'}` 提前收场 —— 不会双倍烧模型,但界面会莫名其妙地
       * 报错/半途而废,看起来像坏了)。
       * 修法:**把语义从"总时长上限"改成"无进展容忍上限"** —— 每次有实质进展(判定循环
       * 每轮回来一次)就 `lingBusyKick()` 续期。这样"既不 resolve 也不 reject 的挂死"
       * (C#9 要防的那件事)照样 3 分钟解除,而"活着、只是在跑很久"不再被误判成挂死。
       */
      var LING_BUSY_MS = 180000; // 3 分钟:**无进展**容忍上限(不是总时长上限 —— 见上)
      /** 判定循环(multi-round)另给一个更宽的**单轮**窗口(审计 E14 的算术):
       *  后端一个 `/conflicts/judge` 请求的整批上限是 `BATCH_MS.judge = 120s`
       *  (host/api.js:92),到点后**在飞的那一条仍会收尾**(单条模型调用最坏 ~90s)
       *  ⇒ 单轮最坏 ≈ 210s > 180s。窗口若小于单轮最坏时长,"还在正常跑的一轮"就会被
       *  误判成挂死。故 judge 取 240s(留余量),并且每轮回来都续期(lingBusyKick)——
       *  于是 40 轮的总时长不再受这个窗口限制,受限制的只是"单轮有没有进展"。 */
      var LING_BUSY_JUDGE_MS = 240000;
      var lingBusyUntil = 0;
      var lingBusyWindow = LING_BUSY_MS; // 当前这把锁用的窗口(kick 续期时按它续)
      /** 上锁(顺手记窗口)。三个按钮共用,免得"上锁"这件事被抄成三份、改一处漏两处。 */
      function lingBusySet(ms) {
        lingBusy = true;
        lingBusyWindow = ms || LING_BUSY_MS;
        lingBusyUntil = Date.now() + lingBusyWindow;
      }
      /** 有实质进展就续期(审计 E14)。只有"忙锁真的持着"时才续,免得把一个已解除的锁又续回来。 */
      function lingBusyKick() {
        if (lingBusy) lingBusyUntil = Date.now() + lingBusyWindow;
      }
        ...
        judge: function (onProgress) {
          ...
          var step = function () {
            ...
            return api('/conflicts/judge', { method: 'POST', body: { limit: 5 } }).then(function (r) {
              // 审计 E14:一轮回来了 = 一次实质进展 ⇒ 给忙锁续期(不然 40 轮的长跑会被
              // 3 分钟的无进展兜底误判成"挂死",自动解锁后就能并发起第二个判定循环)。
              lingBusyKick();
              // 1.5.2 复核补注:这次续期**故意**留在 `!r.ok` 判定之前 —— 失败轮也会续一次。
              // 判定依据(不是"顺手":被要求判断后才保留的):下面**每一条**失败路径都立即
              // `return null`(循环不再继续),而调用方在 `.then` / `.catch` **两路**都复位
              // `lingBusy` ⇒ 这次续期活不过一个 tick,不可能留下一把僵尸锁;反过来若把续期
              // 挪到 ok 判定之后,收益为 0(失败即终止,没有"下一轮"需要这把锁),
              // 却要多一层缩进 —— 故保持现状,只在此记录理由。
              if (!r || !r.ok) {
                // 后端对批处理有单实例锁:另一个判定循环在跑时回 `reason:'busy'`
                //(host/api.js:87-105 batchEnter)→ 别把英文内部码丢给主人。
                toast('✗ ' + ((r && r.reason) === 'busy' ? '已有一个判定循环在跑,等它结束再点'
                  : ((r && r.reason) || '模型判定失败')));
                return null;
              }
